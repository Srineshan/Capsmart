...
const ApplicationFieldCard = ({
  ...
  const getValueByPath = (obj, path) => {
    console.log("qwertyuioxdcfvgbhnjv");
  ...
  const changeHandler = async (event) => {
    ...
    try {
      const response = await POST(
        `application-management-service/application/${applicationId}/files/bulk?isLLMRequired=${
          formSchema?.requiredDocuments?.length !== 0 ? true : false
  ...
  let isprivilegeReferences = getValueByPath(
    basicForm,
    isTableEdit
      ? "undefined.privilegeReferences.address.pinCode"
      : "privilegeReferences.address.pinCode"
  );
  let isreferences = getValueByPath(
    basicForm,
    isTableEdit
      ? "undefined.references.address.pinCode"
      : "references.address.pinCode"
  );

  console.log("Privilege Reference PinCode: ", isprivilegeReferences);
  console.log("Reference PinCode: ", isreferences);
  console.log("MMMMMMMMMMMMMMMMMMMMMMM", isprivilegeReferences, basicForm);

  let isMailingAddressSameAsHomeAddress = getValueByPath(
    basicForm,
    "forms[1].data.contactAddress2.isMailingAddressSameAsHomeAddress"
  );
  let isBusinessAddressSameAsHomeAddressOrMailingAddress = getValueByPath(
    basicForm,
    "forms[1].data.contactAddress3.isBusinessAddressSameAsHomeAddressOrMailingAddress"
  );
  let isHomeAddressPincodeEntered = getValueByPath(
    basicForm,
    "forms[1].data.contactAddress1.homeAddress.pinCode"
  );

  let isMailingAddressPincodeEntered = getValueByPath(
    basicForm,
    "forms[1].data.contactAddress2.mailingAddress.pinCode"
  );
  let isBusinessAddressPincodeEntered = getValueByPath(
    basicForm,
    "forms[1].data.contactAddress3.business.businessAddress.pinCode"
  );
  let registeredBusinessAddress = getValueByPath(
    basicForm,
    "forms[1].data.contactAddress3.registeredBusinessAddress"
  );
      ...
      setBasicForm((prevData) => {
        ...
        if (isMailingAddressSameAsHomeAddress) {
          tempBasicForm.forms[1].data.contactAddress2.mailingAddress.streetName =
            tempBasicForm?.forms[1]?.data?.contactAddress1?.homeAddress
      ...
      setBasicForm((prevData) => {
        ...
        if (
          ...
          tempContactAddress3.forms[1].data.contactAddress3.business.businessAddress.streetName =
            tempContactAddress3?.forms[1]?.data?.contactAddress1?.homeAddress
              ?.streetName !== undefined
              ? tempContactAddress3?.forms[1]?.data?.contactAddress1
          ...
          tempContactAddress3.forms[1].data.contactAddress3.business.businessAddress.streetName =
            tempContactAddress3?.forms[1]?.data.contactAddress2?.mailingAddress
              ?.streetName !== undefined
              ? tempContactAddress3?.forms[1]?.data.contactAddress2
                  ?.mailingAddress?.streetName
              : "";
          tempContactAddress3.forms[1].data.contactAddress3.business.businessAddress.pinCode =
            tempContactAddress3?.forms[1]?.data.contactAddress2?.mailingAddress
              ?.pinCode !== undefined
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setBasicForm((prevData) => {
          let tempContactAddress1 = { ...prevData };
          tempContactAddress1.forms[1].data.contactAddress1.homeAddress.city =
            data?.standard?.city || "";
          tempContactAddress1.forms[1].data.contactAddress1.homeAddress.province =
            data?.standard?.prov || "";
          return tempContactAddress1;
        });
      } catch (error) {
    ...
    const fetchData = async () => {
      try {
        ...
        setBasicForm((prevData) => {
          ...
          tempContactAddress2.forms[1].data.contactAddress2.mailingAddress.city =
            data?.standard?.city || "";
          tempContactAddress2.forms[1].data.contactAddress2.mailingAddress.province =
            data?.standard?.prov || "";
  ...
  const addNewDocument = async (file) => {
    ...
    if (file !== null) {
      ...
      try {
        const response = await POST(
          `application-management-service/application/${applicationId}/files?isLLMRequired=${
            formSchema?.requiredDocuments?.length !== 0 ? true : false
          }&schemaId=${formSchema?.id}`,
          formData
        );
  ...
  const renderField = (
    ...
    console.log(
      fieldKey,
      "qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq" + JSON.stringify(fieldData),
      ...
      switch (fieldData.fieldType) {
            ...
            return (
              ...
              (user === null ||
                ...
                <div>
                  ...
                  {fieldKey === "applicantType" ? (
                    <Tooltip
                      title={`To change applicant type contact ${
                        basicForm?.createdBy?.name?.firstName
                      } ${
        ...
        case "radiobutton":
          return (
            ...
            >
              <div
                className={`${style.lableRadioStyle} ${
          ...
          return (
            ...
            >
              <div className={style.displayInRow}>
                ...
                <div
                  className={`${style.lableRadioStyle} ${
                    fieldData.serialNumber !== null ? style.marginLeft10 : ""
                  } ${fieldData.label !== null ? style.marginRight : ""}`}
                >
                  {fieldData.label}
                  {(isLableEmpty(fieldData.label)
  ...
  return (
    <>
      ...
      <div
        className={`${
          window.location.pathname.includes("applicationForm") || isPOD
            ? ""
      ...
      >
        ...
        {addMoreType && !collapsableQuestionCard ? (
          ...
          <div
            className={`${gridStyle} ${
              object?.label !== null ? style.marginTop : ""
            }`}
          >
            {object?.type === "object"
              ? renderObjectFields(object, object?.properties)
