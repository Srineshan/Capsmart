...
const DepartmentTrackerDialog = ({ getIsOpen, isLoading ,getActiveApplicationView,getNotesDialog}) => {
  ...
  useEffect(() => {
    getActiveUserData()
  }, [ sortField, sortValue,page,totalCount,selectedDepartment]);
...
const headerValues = [
  ...
  const getActiveUserData = async () => {
    try {
        setIsLoadingImage(true);
        const url = `application-management-service/staff/reappointmentStatusDetails?sortBy=${sortValue}&sortByField=${sortField}&limit=10&offset=${page - 1}${selectedDepartment ? `&departmentId=${selectedDepartment}` : ''}`;
