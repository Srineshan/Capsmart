...
const ApplicationAssistanceCard = ({ user, designation, contactNumber, email }) => {
    return (
        <div className={style.applicationAssistanceCard}>
            ...
            <div className={`${style.displayInRow} ${style.marginTop} ${style.cursorPointer}`} onClick={() => window.location.href = "mailto:ngrealy@cmh.org"}><img src={MailIcon} alt="" className={style.iconStyle} /><span className={`${style.contactStyle} ${style.marginLeft} ${style.purpleText}`}> ngrealy@cmh.org</span> </div></Tooltip>
