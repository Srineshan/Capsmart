...
const ESignDialog = ({
  ...
  const handleSubmitApplicationReq = async (data) => {
    ...
    await PUT(
      `application-management-service/application/${applicationId}/form/${basicForm?.forms?.[0]?.id}`,
      temp
    )
      .then((response) => {
      ...
      <div>
        <div className={Classes.DIALOG_BODY}>
          ...
          <p className={`${style.description} ${style.marginTop}`}>
            CAPSmart uses Electronic Signatures for you to sign off on the
            required forms and documents that are part of this application. Draw
            or type your signature below to set it up for use:
          </p>
          <div className={`${style.displayInRow} ${style.marginTop}`}>
            <div>
              <div
                className={`${style.drawOrTypeTextStyle}`}
                onClick={() => {
                  setSelectedESignFormat("DRAW");
                }}
              >
                {" "}
                DRAW
            ...
            <div>
              <div
                className={`${style.drawOrTypeTextStyle} ${style.marginLeft}`}
                onClick={() => {
                  setSelectedESignFormat("TYPE");
                }}
              >
                {" "}
                TYPE
          ...
          {selectedESignFormat === "DRAW" ? (
            <div
              className={`${style.eSignBox} ${style.marginTop} ${style.cursorPointer}`}
              onClick={
            ...
            <div
              className={`${style.eSignBox} ${style.marginTop}`}
              onClick={!isShowType ? () => setIsShowType(true) : () => {}}
            >
              {/* {!isShowType ? (
                                <div className={style.verticalAlignCenter}>
