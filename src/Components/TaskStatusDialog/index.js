...
const TaskStatusDialog = ({ getIsOpen }) => {
  ...
  const tasksendapplication = async (taskId, status, label) => {
    ...
    await POST(
      `application-management-service/application/${id}/task/${taskId}/execute`,
      data
    )
      .then((response) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className={style.task}>
                      {taskData?.taskName}
                      {taskData?.taskName === "Logistics Form for IT" && (
                        <div
                          className={style.requestForm}
                          onClick={() =>
                            window.open(taskData?.formLink?.url, "_blank")
                          }
                        >
                          {taskData?.formLink?.urlLabel?.text}
                        </div>
                      )}
                    </div>
                    <div>
                      {showSelect ? (
                        <div className={style.sentstatus}>
                          ...
                          <div>
                            <CommonSelectField
                              value={
                                selectedOption[taskData.id] ||
                                taskData?.taskUpdateStatus?.status
                              }
                              onChange={(e) =>
                                handleChange(
                                  taskData.id,
                                  e,
                                  taskData?.statusLabels
                                    ?.filter(
                                      (data) => e.target.value === data.status
                                    )
                                    ?.map(
                              ...
                              valueList={taskData?.statusLabels.map(
                                (statusLabel) => `${statusLabel?.status}`
                              )}
                              labelList={taskData?.statusLabels.map(
                                (statusLabel) => `${statusLabel?.label}`
                              )}
                              disabledList={taskData?.statusLabels.map(
                                () => false
                              )}
                            />
                          </div>
                        ...
                        <div className={style.sentto}>
                          {isNotCompleted
                            ? "Ready To Send"
                            : `${taskData?.activityExecutionPromptLabel?.text} on ${formattedDate}`}
                        </div>
                      )}
                    </div>
                    <div>
                      {showSelect ? (
                        ...
                        <div
                          ...
                          onClick={() =>
                            tasksendapplication(
                              taskData?.id,
                              taskData?.statusLabels?.status,
                              taskData?.statusLabels?.label
                            )
                          }
                        >
                          {isNotCompleted
