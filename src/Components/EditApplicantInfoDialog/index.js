...
const EditInfoDialog = ({ checkedIds, getIsOpen, onClose,applicationId,selectedEditField }) => {
  ...
  console.log("selectedDepartmentService",selectedDepartmentService,selectedEditField,selectedDepartmentName,selectedSpecialtyName,businessAddressNeed,businessName,"s112",initialForm?.businessName)
  ...
  setSameAddress(form?.applicant?.contactAddress2?.sameAsHomeAddress === true ? "SAME_CURRENT" : "DIFFERENT_ADDRESS")
}, [form]);
...
useEffect(() => {
  ...
  setIsApproveEnabled(hasChanged);
}, [
  firstName,
  middleName,
  lastName,
...
useEffect(() => {
  ...
  if (selectedDeptObj?.serviceAreas?.length > 0) {
    ...
    temp.applicant.contactAddress3.business.sameAsMailingAddress = sameAddressInMailing === "SAME_MAILING" ? true : false;
  } else {
    ...
    setUpdateForm(updateBasicform);
    setIsLoadingImage(false);
    SuccessToaster2("Successfully Updated");
    getIsOpen(false);
  } catch (error) {
      ...
      <div>
            ...
            <div>
              ...
              <CommonRadio
                  ...
                  radioValue={["SAME_CURRENT","SAME_MAILING","DIFFERENT_ADDRESS"]}
                  label={["Same as current home address?","Same as Mailing address?","different address"]
                  }
                />
              </div>
                    )}
