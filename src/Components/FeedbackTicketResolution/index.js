...
const FeedbackTicketResolution = ({
  getShowFeedbackTicketResolution,
  ticketId,
  isEdit,
}) => {
  ...
  const [dateAndTime, setDateAndTime] = useState(
    formatInTimeZone(new Date(), siteTimeZone(), "MM-dd-yyyy HH:mm")
  );
  const [modifiedDateAndTime, setModifiedDateAndTime] = useState(
    formatInTimeZone(new Date(), siteTimeZone(), "MM-dd-yyyy HH:mm")
  );
  const [showFeedbackTicketResolutionLog, setShowFeedbackTicketResolutionLog] =
    useState(false);
  ...
  useEffect(() => {
    setCurrentUserData(
      users
        ?.filter((data) => data?.id === loggedInUser?.id)
        ?.map((data) => data)
    );
  }, [users]);
  ...
  useEffect(() => {
    if (screenCaptured) {
      setFileName(
        `${currentUserData?.[0]?.id}${new Date().getTime().toString()}.${
          match[1]
        }`
      );
    }
  ...
  useEffect(() => {
    if (
      ...
      setMatch(
        screenCapture !== ""
          ? screenCapture.match(/^data:image\/(\w+);base64,/)
          : []
      );
      getImgBlob();
    }
  ...
  useEffect(() => {
    if (ticketDetails) {
      ...
      setDateAndTime(
        formatInTimeZone(
          new Date(ticketDetails?.createdDateTime),
          siteTimeZone(),
          "MM-dd-yyyy HH:mm"
        )
      );
      setModifiedDateAndTime(
        formatInTimeZone(
          new Date(ticketDetails?.modifiedDateTime),
          siteTimeZone(),
          "MM-dd-yyyy HH:mm"
        )
      );
      ...
      if (assignToTemp === "" || assignToTemp === null) {
        setAssignToId(
          ticketDetails?.assignedTo?.id !== null
            ? ticketDetails?.assignedTo?.id
            : ""
        );
        handleAssignTo(
          ticketDetails?.assignedTo?.id !== null
  ...
  const getUser = async () => {
    const { data: user } = await GET(
      `user-management-service/user/ListOfId?userIds=${userIdList}`
    );
    const { data: entityUsers } = await GET(
      `user-management-service/user/role?role=Entity Sys User&role=Entity Sys Admin`
    );
    if (user?.length !== 0) {
      setUsers(user);
    }
  ...
  const getTicketById = async () => {
    const { data: ticket } = await GET(
      `feedback-management-service/ticket/${ticketId}`
    );
    setTicketDetails(ticket);
  };

  const getWorkflowActions = async () => {
    const { data: workflowActions } = await GET(
      `feedback-management-service/ticket/${ticketDetails?.status}/workFlowActions`
    );
  ...
  const getComments = async () => {
    const { data: comments } = await GET(
      `feedback-management-service/ticket_comment?ticketId=${ticketId}`
    );
  ...
  const getImgBlob = async () => {
    ...
    setBlobFormat(
      base64ToUint8Array(screenCapture.replace(/^data:image\/\w+;base64,/, ""))
    );
  };
  ...
  const handleWorkflowUpdate = async () => {
    if (notes !== "") {
      ...
      await PUT(
        `feedback-management-service/ticket/${ticketId}/${ticketStatus}`,
        data
      )
        .then((response) => {
  ...
  const handleSave = async () => {
    ...
    let data = {
      ...
      contractorName: isEdit
        ? ticketDetails?.contractorName
        : `${currentUserData?.[0]?.name?.firstName} ${currentUserData?.[0]?.name?.lastName}`,
      assignedTo: {
        id: assignTo?.[0]?.id,
      ...
      ...(isEdit && { messageCount: ticketDetails?.messageCount }),
      ...
      ticketFile: {
        ...
        ...(isEdit && { id: ticketDetails?.ticketFile?.id }),
        ...(isEdit && { filePath: ticketDetails?.ticketFile?.filePath }),
        ...(isEdit && { fileURL: ticketDetails?.ticketFile?.fileURL }),
      },
      deviceDetails: {
        browser: browser,
  ...
  console.log(
    "users",
    users?.map((data) => data?.profilePic?.file?.fileURL)
  );

  return (
    <>
      <Dialog
        isOpen={getShowFeedbackTicketResolution}
        onClose={() => handleClose()}
        className={`${style.addManagerDialogBackground} ${style.feedbackDialog}`}
        ref={componentRef}
        ...
        <div className={`${Classes.DIALOG_BODY} `}>
          ...
          <div className={style.spaceBetween}>
            <p className={style.extensionStyle}>
              Feedback Ticket Resolution Progress
            </p>
            <PrintOutlinedIcon
          ...
          <div className={isEdit ? style.feedbackGrid : ""}>
            <div>
              <div
                className={`${style.feedbackContentGrid} ${style.marginTop20}`}
              >
                <div>
                  <p className={style.extentionLableStyle}>Ticket ID</p>
                  <p className={style.feedbackFontStyle}>
                    {isEdit ? ticketName : "-"}
                  </p>
                </div>
                <div className={style.marginLeft}>
                  ...
                  <p
                    className={style.feedbackFontStyle}
                  >{`${dateAndTime} ${timeZoneAbbreviation()}`}</p>
                </div>
                <div>
                ...
                {isEdit && ticketDetails?.generationMode === "SYSTEM" && (
                  <>
                    <div>
                      ...
                      <p
                        className={`${style.feedbackFontStyle} ${style.feedbackMessageSize}`}
                      >
                        {ticketDetails?.exceptionDetails?.trace}
                      </p>
                    </div>
                    <div className={style.marginLeft}>
                      <p className={style.extentionLableStyle}>
                      ...
                      <p
                        className={`${style.feedbackFontStyle} ${style.feedbackMessageSize}`}
                      >
                        {ticketDetails?.exceptionDetails?.message}
                      </p>
                    </div>
                  </>
                )}
              ...
              <div
                className={`${style.extensionBorder} ${style.marginTop20}`}
              ></div>
              <div
                className={`${style.feedbackContentGrid} ${style.marginTop20}`}
              >
                ...
                <div className={style.twoCol}>
                  ...
                  <div>
                    ...
                    {ticketDetails?.generationMode === "SYSTEM" ? (
                      <p className={style.feedbackFontStyle}>
                        {screenCaptured ? "YES" : "NO"}
                      </p>
                    ) : (
                      <FormControlLabel
                        control={
                          <Switch
                            checked={screenCaptured}
                            size="small"
                            onChange={(e) => {
                              ticketDetails?.generationMode !== "SYSTEM" &&
                                setScreenCaptured(e.target.checked);
                            }}
                            disabled={
                              screenCaptureImg !== "" &&
                              screenCaptureImg !== null
                                ? false
                                : true
                            }
                          />
              ...
              <div className={style.dashedBorder}>
                {ticketDetails?.generationMode === "SYSTEM" ? (
                  <div
                    className={`${style.imageDisplayStyle} ${style.alignCenter} ${style.imageNameStyle}`}
                  >
                    IMAGE.PNG
                  </div>
                ) : (
                  <div
                    className={`${style.imageDisplayStyle} ${style.alignCenter}`}
                  >
                    {isEdit && ticketDetails?.ticketFile?.fileURL !== null ? (
                      <img
                        src={ticketDetails?.ticketFile?.fileURL}
                        alt="Screen shot img"
                        className={style.screenCaptureImgStyle}
                      />
                    ) : !screenCaptured && screenCapture === null ? (
                      <>
                        <label
                          htmlFor="file-upload-help"
                          className={`${style.uploadButton} ${style.alignCenter}`}
                        >
                          UPLOAD
                        </label>
                        <input
                      ...
                      <img
                        src={
                          !fromUpload ? screenCapture : screenCaptureFromUpload
                        }
                        alt="Screen shot"
                        className={style.screenCaptureImgStyle}
                      />
...
' className={style.marginLeft20} />
            ...
            <div className={style.spaceBetweenColumn}>
              <div>
                {isEdit && (
                  <>
                    <div
                      className={`${style.collapseHeader} ${style.marginTop20} ${style.spaceBetween}`}
                    >
                      <div className={style.displayInRow}>
                        <p className={style.collapseHeaderText}>
                          Ticket Status
                        </p>
                        <div
                          className={`${
                            ticketStatus !== "In-Progress"
                              ? style.greenDotFeedbackStyle
                              : style.orageDotFeedbackStyle
                          } ${style.marginLeft20}`}
                        ></div>
                      </div>
                      <div className={style.displayInRow}>
                        <p className={style.feedbackFontStyle}>
                          Updated On{" "}
                          {`${modifiedDateAndTime} ${timeZoneAbbreviation()}`}
                        </p>
                        <Icon
                          icon="chevron-up"
                          color="#06617A"
                    ...
                    <div className={style.collapseBody}>
                      <div className={`${style.extentionGrid}`}>
                        <div
                          className={`${style.extentionLableStyle} ${style.marginTop10}`}
                        >
                          Assign To
                        </div>
                        <div className={style.displayInRow}>
                          ...
                          <CommonSelectField
                            ...
                            onChange={(e) => {
                              ...
                              handleAssignTo(e.target.value);
                            }}
                            className={`${style.fieldWidth2InARow}`}
                            firstOptionLabel={"Select Assignee"}
                            firstOptionValue={""}
                            valueList={entityAndSiteLevelUsers?.map(
                              (data) => data?.id
                            )}
                            labelList={entityAndSiteLevelUsers?.map(
                              (data) =>
                                `${data?.name?.firstName} ${data?.name?.lastName}`
                            )}
                            disabledList={entityAndSiteLevelUsers?.map(
                              (data) => false
                            )}
                          />
                        </div>
                      ...
                      <>
                        {assignToId !== "" && assignToId !== null && (
                          <div
                            className={`${style.extentionGrid} ${style.marginTop10}`}
                          >
                            <div></div>
                            <div>
                              {
                                entityAndSiteLevelUsers?.filter(
                                  (data) => data?.id === assignToId
                                )[0]?.email?.officialEmail
                              }{" "}
                              <span
                                className={`${style.marginLeft20} ${style.marginRight}`}
                              >
                                {" "}
                                |{" "}
                              </span>{" "}
                        ...
                        >
                          ...
                          <div>
                            ...
                            <div>
                              {ticketDetails?.createdBy?.email?.officialEmail}{" "}
                              <span
                                className={`${style.marginLeft20} ${style.marginRight}`}
                              >
                                {" "}
                              ...
                              {ticketDetails?.createdBy?.communication
                                ?.mobileNumber !== ""
                                ? ticketDetails?.createdBy?.communication
                                    ?.mobileNumber
                                : "-"}
                        ...
                        >
                          ...
                          <div>
                            <div>
                              {statusAvailableValues[ticketDetails?.status]}
                            </div>
                          </div>
                        </div>
                      </>
                      ...
                      >
                        ...
                        <div className={style.displayInRow}>
                          ...
                          <CommonSelectField
                            ...
                            className={`${style.fieldWidth2InARow}`}
                            firstOptionLabel={"Select Status"}
                            firstOptionValue={""}
                            valueList={workflowActions}
                            labelList={workflowActions?.map(
                              (data) => workflowActionAvailableValues[data]
                            )}
                      ...
                      >
                        ...
                        <TextArea
                          ...
                          rows={3}
                          className={`${style.fullWidth} ${style.transparentBackground} ${style.transparentNotesBorder}`}
                        />
                      </div>
                      ...
                      <div
                        className={`${style.resolutionProgressCard} ${style.spaceBetween} ${style.verticalAlignCenter} ${style.marginTop20} ${style.cursorPointer}`}
                        onClick={() => setShowFeedbackTicketResolutionLog(true)}
                      >
                        <div
                          className={`${style.resolutionProgressTextStyle} ${style.marginLeft20}`}
                        >
                    ...
                    {isEdit && (
                      <>
                        ...
                        <div className={style.collapseBody}>
                          {allComments?.map((data, index) => (
                            <div>
                              <div className={style.commentGrid}>
                                <div>
                                  {(index === 0 ||
                                    allComments[index - 1]?.commentedBy?.id !==
                                      data?.commentedBy?.id) && (
                                    <img
                                      src={
                                        users
                                          ?.filter(
                                            (user) =>
                                              user?.id === data?.commentedBy?.id
                                          )
                                          ?.map(
                                            (user) =>
                                              user?.profilePic?.file?.fileURL
                                          )[0] || DoctorAnime
                                      }
                                      alt="logo"
                                ...
                                <div>
                                  {(index === 0 ||
                                    ...
                                    <div className={style.marginLeft20}>
                                      <div
                                        className={`${style.displayInRow} ${style.marginTop10} ${style.commentTextStyle}`}
                                      >
                                        {data?.commentedBy?.name?.firstName}{" "}
                                        {data?.commentedBy?.name?.lastName}
                                        <span
                                          className={`${style.blue} ${style.marginLeft20}`}
                                        >
                                          {" "}
                                          {
                                            data?.commentedBy?.name?.suffix
                                              ?.suffix
                                          }
                                        </span>{" "}
                                        <span
                                          className={`${style.greyText} ${style.marginLeft20}`}
                                        >
                                          {getDaysAgo(
                                  ...
                                  <div
                                    className={`${style.marginTop10} ${style.marginLeft20} ${style.commentTextStyle}`}
                                  >
                                    {data?.comment}
                                  </div>
                                </div>
                          ...
                          />
                          <div
                            className={`${style.alignRight} ${style.marginTop10}`}
                          >
                            <button
                              className={`${style.sendButton} ${
                                comment === "" ? style.disabledView : ""
                              }`}
                              onClick={() =>
      ...
      {showFeedbackTicketResolutionLog && (
        <FeedbackTicketResolutionLog
          getShowFeedbackTicketResolutionLog={
            getShowFeedbackTicketResolutionLog
          }
