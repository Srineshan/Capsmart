...
const AddNewDepartments = ({getAddDeptEntityDialog}) => {

    return(
        <Dialog isOpen={getAddDeptEntityDialog} onClose={() => getAddDeptEntityDialog(false)} className={`${style.healthCareDialogStyle} ${style.dialogPaddingBottom}`}>
          <div className={`${Classes.DIALOG_BODY} ${style.extensionDialogBackground}`}>
            <div className={style.spaceBetween}>
                <p className={style.extensionStyle}>New Departments / Services Area For Hospital / Acute Care Facility (ACF)</p>
                <Icon icon="cross" size={20} intent={Intent.DANGER} className={style.dialogCrossStyle} onClick={() => getAddDeptEntityDialog(false)}  />
            </div>
