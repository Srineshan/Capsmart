...
const AddContractedServiceForHospital = ({ getAddContractedServiceDialog, isEdit, selectedContractedServiceProvider, entityType, siteTypeId, getContractedServiceProvider }) => {
    ...
    useEffect(() => {
        if (isEdit) {
            setContractedServiceProviderType(selectedContractedServiceProvider?.contractedServiceProviderType);
        } else {
    ...
    const saveSubmitHandler = async (type) => {
        const data = {
            ...
            ...(isEdit && { createdDate: selectedContractedServiceProvider?.createdDate }),
    ...
    return (
        <Dialog isOpen={getAddContractedServiceDialog} onClose={() => { getAddContractedServiceDialog(false); setContractedServiceProviderType('') }} className={`${style.healthCareDialogStyle} ${style.dialogPaddingBottom}`}>
            <div className={`${Classes.DIALOG_BODY} ${style.extensionDialogBackground}`}>
                <div className={style.spaceBetween}>
                    ...
                    <Icon icon="cross" size={20} intent={Intent.DANGER} className={style.dialogCrossStyle} onClick={() => { getAddContractedServiceDialog(false); setContractedServiceProviderType('') }} />
                </div>
                ...
                <div className={`${style.addHealthCareBoxStyle}`}>
                    <div className={`${style.extentionGrid}`}>
                        ...
                        <div className={style.displayInRow}>
                            <InputGroup value={entityType} className={style.fullWidth} rightElement={arrowDown()} disabled={true} />
                        </div>
                    </div>
                    ...
                    <div className={`${style.addHealthCareBoxStyle}`}>
                        <div className={`${style.absenseCareGrid2}`}>
                            ...
                            <div className={style.displayInRow}>
                                <InputGroup value={contractedServiceProviderType} onChange={(e) => setContractedServiceProviderType(e.target.value)} className={style.fullWidth} />
                            </div>
                        </div>
                    </div>
                    {!isEdit && (
                        <div className={`${style.spaceBetween} ${style.marginTop20}`}>
                            ...
                            <div className={`${style.addMoreCardStyle} ${style.addMoreTextStyle}`} onClick={() => saveSubmitHandler("Add More")}>ADD MORE</div>
                        </div>
                    )}
                </div>
                <div>
