...
const AddTerminationReasonsForCustomer = ({
    ...
    const [terminationId, setTerminationId] = useState(selectedTermination?.id ? selectedTermination?.id : "");
    ...
    useEffect(() => {
        if (isEdit) {
            ...
            setAddSubReasons(selectedTermination?.secondary_reasons?.length > 0 ? true : false)
            if (isSecondary) {
    ...
    const getSubReasons = () => {
        ...
        for (let i = 0; i < secondaryReasonList?.length; i++) {
            console.log(i)
            temp[i] = (
                <div className={`${style.editHealthCareGrid2}`} key={`${i}${secondaryReasonList[i]}`}>
