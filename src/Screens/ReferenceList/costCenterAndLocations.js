...
const CostCenterAndLocations = () => {
    ...
    const [showAddCostCenterLocation, setShowAddCostCenterLocation] = useState(false);
    ...
    const handleLocationStatusUpdate = async (value, data, index) => {
        ...
        console.log(temp);
        await PUT(
            `entity-service/costCenter/${data?.id}`,
            JSON.stringify(temp)
        )
            .then((response) => {
    ...
    return (
        <Fragment>
                    ...
                    <div>
                        ...
                        <div className={style.marginTop35}>
                            <div className={style.centreCardStyle}>
                                <div className={style.margin20}>
                                    <div className={style.costCenterGrid}>
                                        <div>
                                            <div className={`${style.costCenterLeftCard} ${style.spaceBetween}`}>
                                                <div className={`${style.costCenterLeftCardText}`}>SMMC</div>
                                                <div className={`${style.costCenterLeftCardText}`}>{costCenter?.length} </div>
                                            </div>
                                        </div>
                          ...
                          icon
                                            ...
                                            <div>
                                                <div className={`${style.customListTopHeaderForCostCenter} ${style.spaceBetween}`}>
                                                    <p
                                                        className={
                                                    ...
                                                    <img
                                                        ...
                                                        onClick={() => setShowAddCostCenterLocation(true)}
                                                    />
                                                </div>
                                                {costCenter?.map((data, index) => (
                                                    <>
                                                        <div className={style.customListHeaderForCostCenter}>
                                                            <img
                                                            ...
                                                            <img
                                                                ...
                                                                onClick={() => { setSelectedCostCenterLocation(data); getAddCostCenterDialog(true); setIsEdit(true); }}
                                                            />
                                                            {/* <img
                                                                src={DeleteHcRow}
                                                                alt="OpenFolder"
                                                        ...
                                                        {data?.serviceLocations?.map((locationData, locationIndex) => (
                                                            <div key={locationIndex}>
                                                                <div className={style.costCenterLocationCard}>
                                                                    <div className={`${style.costCenterFontStyle} ${style.verticalAlignCenter}`} > {locationData?.location} </div>
                                                                    <div className={`${style.costCenterFontStyle} ${style.verticalAlignCenter}`} > {locationData?.code} </div>
                                                                    <CommonSelectField
                                                                        value={locationData?.status}
                                                                        onChange={(e) => handleLocationStatusUpdate(e.target.value, data, locationIndex)}
                                                                        firstOptionLabel={''} firstOptionValue={''}
                                                                        valueList={['ACTIVE', 'INACTIVE']}
                                                                        labelList={['ACTIVE', 'INACTIVE']}
                                                                        disabledList={[false, false]} />
            ...
            {showAddCostCenterLocation && (
                <AddCostCenterAndLocations getAddCostCenterDialog={getAddCostCenterDialog} selectedCostCenterLocation={selectedCostCenterLocation} isEdit={isEdit} getCostCenterData={getCostCenterData}
                />
            )}
        </Fragment>
    );
