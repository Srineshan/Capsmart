...
const EntitySystemAdmin = ({ getActiveStep }) => {
  ...
  const [selectedRoles, setSelectedRoles] = useState(
    roles
      ?.filter((data) => data?.roleName === "Entity Sys Admin")
      ?.map((data) => data)
  );
  ...
  const [customerAccountManagerChecked, setCustomerAccountManagerChecked] =
    useState(true);
  ...
  const getUserData = async () => {
    // await axios(`http://ec2-34-230-167-131.compute-1.amazonaws.com:8010/user-management-service/user?partnerId=${partnerId}&userType=PARTNER_USER`, {
    //   method: 'GET',
    //   headers: {
    ...
    const { data: users } = await GET(
      `user-management-service/user?partnerId=${partnerId}&userType=PARTNER_USER`
    );
    setUsers(users);
  };
  ...
  const handleUpdate = async (buttonType) => {
    if (customerAccountManagerChecked || createUserChecked) {
      let temp = entity;
      let managerDetails = users
        ?.filter((data) => data?.id === accountManager)
        ?.map((data) => data)?.[0];
      temp.accountManager = {
        id: accountManager,
  ...
  return (
    <>
      {isSetupComplete ? (
        <SetupComplete
          data={
            entity?.subscriptionPlan?.planName === "TRIAL"
              ? "Trial"
              : "Customer"
          }
        ...
        <div className={style.entitySetupBackground}>
          ...
          <div className={style.stepperMargin}>
            <div className={style.stepperGrid}>
              ...
              >
                ...
                <p
                  className={`${style.entityTextColor} ${style.activeEntityTextColor}`}
                >
                  SUBSCRIPTION PLAN
                </p>
              </div>
              ...
              <div
                onClick={() =>
                  ...
                  navigate(`/entitySetup/${id}/siteInformation`)
                }
                className={!entity?.multiSiteEntity && style.disabledView}
              >
                <div className={style.justifyCenter}>
                  <div
                    className={`${style.stepperImgBackground} ${style.completedStepperStyle}`}
              ...
              <div
                onClick={() => navigate(`/entitySetup/${id}/entitySystemAdmin`)}
              >
                <div className={style.justifyCenter}>
                  <div
          ...
          <div className={style.entitySetupCardStyle}>
            ...
            <div className={style.entityDescription}>
              In this step provide the person that will be responsible for this
              account from your organization. Also setup the customer sys admin
              user.
            </div>
            <div>
              <div className={style.cloneBlockStyle}>
                <div className={`${style.newContractFromCloneBoxStyle}`}>
                  ...
                  <div
                    className={`${style.extentionGrid} ${style.marginTop30}`}
                  >
                    <div className={style.extentionLableStyle}>
                      Available Account Manager*
                    </div>
                    <div className={`${style.leftAlign} `}>
                      ...
                      <CommonSelectField
                        ...
                        onChange={(e) => setAccountManager(e.target.value)}
                        className={`${style.fullWidth}`}
                        firstOptionLabel={"Select Account Manager"}
                        firstOptionValue={""}
                  ...
                  <div
                    className={`${style.textAlignLeft} ${style.marginTop20}`}
                  >
                    <CommonCheckBox
                  ...
                  >
                    ...
                    <div className={style.displayInRow}>
                      <InputGroup
                        className={style.textFieldWidth}
                ...
                >
                  <button
                    ...
                    onClick={() => saveInProgressCheck()}
                  >
                    SAVE IN-PROGRESS
                  </button>
                  <button
                    className={`${style.buttonStyle} ${style.marginLeft20}`}
                    onClick={() => handleUpdate("Continue")}
                  >
                    CONTINUE
                  </button>
                </div>
