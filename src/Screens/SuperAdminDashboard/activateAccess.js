...
const ActivateAccess = () => {
  ...
  const [isSmallCharacterAvailable, setIsSmallCharacterAvailable] =
    useState(false);
  const [isCapitalCharacterAvailable, setIsCapitalCharacterAvailable] =
    useState(false);
  const [isNumberAvailable, setIsNumberAvailable] = useState(false);
  const [isSpecialCharacterAvailable, setIsSpecialCharacterAvailable] =
    useState(false);
  const [isMin8CharacterAvailable, setIsMin8CharacterAvailable] =
    useState(false);
  ...
  useEffect(() => {
    let length = [
      isCapitalCharacterAvailable,
      isSmallCharacterAvailable,
      isMin8CharacterAvailable,
      isNumberAvailable,
    ...
    return (
      <div className={style.activateAccessBackground}>
        <div className={style.activateAccessCard}>
          <div className={style.activateHeaderText}>
            <strong>Time</strong>Smart.AI
          </div>
          {entityLogo?.file?.fileURL && (
            <img
              src={entityLogo?.file?.fileURL}
              alt=""
        ...
        <InputGroup type="email" large={true} value={users?.filter(data => data?.id === randomId)?.map(data => data?.email?.officialEmail)[0]} className={style.marginTop10} /> */}
          <div
            className={`${style.regHeading} ${style.blackText} ${style.marginTop30} `}
          >
            To Activate Your TimeSmartAI Access Using Single Sign-On, Provide Your
            OKTA Username
          </div>
          <InputGroup
            type={"text"}
            ...
            className={`${style.marginTop10} ${confirmUsername?.length > 0 &&
          ...
          />
          <p
            className={`${style.textAlignLeft} ${style.marginTop10} ${style.activateAccessHelpText}`}
          >
            Make sure the Username you have provided is what you use for accessing
            your Single Sign-On Portal{" "}
          </p>
          ...
          <InputGroup
            type={"text"}
            large={true}
            placeholder="Confirm Username provided"
            className={`${style.marginTop20} ${confirmUsername?.length > 0 &&
              confirmUsername !== username &&
              style.redBorderField
              } ${confirmUsername?.length > 0 &&
              confirmUsername === username &&
          ...
          {confirmUsername?.length > 0 && confirmUsername !== username && (
            <p
              className={`${style.marginTop10} ${style.activateAccessAlertText}`}
            >
              Username does not match, Check & re-enter
            </p>
          )}
          {
          ...
          <button
            className={`${style.loginButton} ${style.marginTop30}`}
            onClick={() => {
              handleUsernameCheck();
            }}
          >
