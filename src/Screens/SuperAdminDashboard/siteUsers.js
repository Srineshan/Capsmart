...
const SiteUsers = ({ getActiveStep }) => {
  ...
  const getContracts = async () => {
    await axios(`http://ec2-18-232-204-138.compute-1.amazonaws.com:8010/contract-managment-service/contracts`, {
      method: 'GET',
      headers: {
  ...
  return (
    <div className={style.entitySetupBackground}>
      <Icon
        ...
        onClick={() => navigate("/activeCustomers")}
      />
      ...
      {showUserTable ? (
        <div className={style.entitySetupCardStyle}>
          ...
          <div className={style.entityDescription}>
            ...
            finibus quam nec tellus dictum, vitae ultrices urna porttitor. donec
            commodo tellus dapibus semper mattis. aenean ut massa vitae tortor
            consequat tristique. etiam eget condimentum sapien. morbi est ante,
            sagittis ac rhoncus eget, faucibus ut felis. pellentesque iaculis
            aliquam massa. lorem ipsum dolor sit amet, consectetur adipiscing
            elit. sed finibus quam nec tellus dictum.
          </div>
          <div>
              ...
              <div className={`${style.newContractFromCloneBoxStyle}`}>
                {!isSuperAdminAccess && (
                  ...
                  >
                    <div className={style.extentionLableStyle}>
                      Is App User A Contractor? *
                    </div>
                    <div className={style.displayInRow}>
                      <FormControlLabel
                        control={
                          <Switch
                            checked={isAppUserContractor}
                            className={` ${style.textAlignLeft} `}
                            onChange={() =>
                              setIsAppUserContractor(!isAppUserContractor)
                            }
                          />
                        }
                ...
                <div className={`${style.extentionGrid} ${style.marginTop20}`}>
                  ...
                  <div className={`${style.displayInRow}`}>
                    ...
                    />
                    <SuffixList
                      value={userData?.suffix?.id}
                      onChangeFunc={onSuffixChange}
                      className={[style.fourFieldWidth, style.marginLeft20]}
                    />
                    <p className={`${style.fourFieldWidth}`}></p>
                  </div>
                ...
                <div className={`${style.extentionGrid} ${style.marginTop20}`}>
                  ...
                  <InputGroup
                    ...
                    onChange={(e) => handleUserData("phone", e.target.value)}
                  />
                </div>
                        ...
                        <FormControlLabel
                          control={
                            <Switch
                              ...
                              className={` ${style.textAlignLeft}`}
                              onChange={() => setSiteSpecific(!siteSpecific)}
                            />
                          }
                        ...
                        {siteSpecific && (
                          <>
                            <DatalistInput
                              ...
                              onSelect={onSelect}
                              className={`${style.fullWidth} ${style.marginLeft20} ${style.textAlignLeft}`}
                            />
                          </>
                        )}
                      </div>
                  ...
                  <FormControl className={style.fullWidth}>
                    ...
                    <Select
                      ...
                      renderValue={(selectedRoles) =>
                        selectedRoles.map((data) => data?.roleName)?.join(", ")
                      }
                    >
                      {entityRoles.map((option) => (
              ...
              <div className={style.spaceBetween}>
                ...
                >
                  ...
                  <button
                    ...
                    onClick={() => mandatoryFieldCheck("AddMore")}
                  >
                    SAVE & ADD MORE
                  </button>
                  <button
        ...
        <div className={style.entitySetupCardStyle}>
            ...
            <div className={`${style.tableData} ${style.displayInCol}`}>
              {user?.map((data) => (
                ...
                >
                  ...
                  <p className={style.tableDataFontStyle}>
                    {data?.sites?.sites?.length || 0}
                  </p>
                  <p className={style.tableDataFontStyle}>
                    {data?.roles
                      ?.map((data) => data?.roleName)
                      .includes("Entity Sys Admin")
                      ? "YES"
                      : "NO"}
                  </p>
                  <p className={style.tableDataFontStyle}>
          ...
          >
            ...
            <button
              className={`${style.buttonStyle} ${style.marginLeft20}`}
              onClick={() => getActiveStep("appSubscription")}
            >
              CONTINUE
            </button>
          </div>
      ...
      <Dialog
        ...
        className={`${style.bulkUploadDialog} ${style.dialogPaddingBottom}`}
      >
        <div
          className={`${Classes.DIALOG_BODY} ${style.bulkUploadDialogBackground}`}
        >
          <div className={style.spaceBetween}>
            <p className={style.extensionStyle}>
              USERS BULK UPLOAD FOR CUSTOMER NAME
            </p>
            <Icon
              icon="cross"
              size={20}
              intent={Intent.DANGER}
              className={style.alertCrossStyle}
                      ...
                      {userDataCSV?.length === 0 ? (
                        <>
                          <img
                            src={CloudUpload}
                            alt="cloud"
                            className={style.uploadImgStyle}
                          />
                          <p className={style.uploadTextStyle}>
                            DRAG AND DROP, OR CLICK TO UPLOAD THE EXCEL TEMPLATE
                          </p>
              ...
              <div className={`${style.spaceBetween} ${style.reduceMarginTop}`}>
                <div className={style.displayInRow}>
                  <img
                    src={Download}
                    alt="download"
                    className={style.downloadImageStyle}
                  />
                  <p className={style.failedTesxtColor}>5 RECORDS FAILED</p>
                </div>
                <p className={`${style.resetStyle} ${style.marginRight20}`}>
          ...
          <div className={`${style.uploadDescription} ${style.marginTop20}`}>
            ...
            <p className={style.uploadDescriptionText}>
              Help lorem ipsum dolor sit amet, consectetur adipiscing elit. sed
              finibus quam nec tellus dictum, vitae ultrices urna porttitor.
              ...
              est ante, sagittis ac rhoncus eget, faucibus ut felis.
              pellentesque iaculis aliquam massa. lorem ipsum dolor sit amet,
              consectetur adipiscing elit. sed finibus quam nec tellus dictum.
            </p>
            <CSVLink
              data={[]}
              headers={columns}
          ...
          <div>
            <div className={`${style.floatRight} ${style.marginTop20}`}>
              <button
                ...
                onClick={() => setShowBulkUploadDialog(false)}
              >
                {isUploaded ? "SAVE & CONTINUE" : "CANCEL UPLOAD"}
              </button>
            </div>
          </div>
        </div>
