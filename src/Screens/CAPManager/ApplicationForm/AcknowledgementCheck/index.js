                        ...
                        <div className={`${style.applicationCardStyle}  ${style.marginTop}`}>
                            <div className={`${style.displayInRow}${style.marginTop20}`}>
                                <div className={`${style.spaceBetween} ${style.marginLeftRight20} ${style.marginTop20} ${style.marginBottom20}`}>
                                    ...
                                    <div className={basicForm?.forms?.filter(data => data?.formCategory === 'Form' || data?.formCategory === 'Disclosure')?.every(item => item.dataStatus === "COMPLETED") ? style.greenDotStyle : basicForm?.forms?.filter(data => data?.formCategory === 'Form' || data?.formCategory === 'Disclosure')?.some(item => item.dataStatus === "SKIPPED_MANDATORY_FIELD") ? style.redDotStyle : basicForm?.forms?.filter(data => data?.formCategory === 'Form' || data?.formCategory === 'Disclosure')?.some(item => item.dataStatus === "PENDING") ? style.greyDotStyle : basicForm?.forms?.filter(data => data?.formCategory === 'Form' || data?.formCategory === 'Disclosure')?.some(item => item.dataStatus === "SKIPPED_NON_MANDATORY_FIELD") ? style.yellowDotStyle : style.greyDotStyle}></div>
                                </div>
                            </div>
