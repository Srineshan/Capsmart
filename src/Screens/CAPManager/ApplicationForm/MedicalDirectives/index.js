...
const MedicalDirectives = ({ basicForm, setBasicForm, applicationId, getPreApplication, dateFormat, name }) => {
    ...
    const handleContinue = async (data) => {
        ...
        let temp = {
            ...
            dataStatus: completedCount === 0 ? 'SKIPPED_MANDATORY_FIELD' : completedCount === totalCount ? 'COMPLETED' : 'SKIPPED_NON_MANDATORY_FIELD'
        }
    ...
    const handleEdit = (data) => {
        navigate(`/applicationForm/${applicationId}/${basicForm?.forms?.[formIndex]?.formCategory}/${btoa(basicForm?.forms?.[formIndex]?.schemaCategory)}/${data?.medicalDirective?.id}`)
    }
    ...
    const getMedicalDirectiveTable = () => {
        ...
        if (medicalDirectivesStatus === 'completed') {
            ...
            temp.push({ "type": "text", "value": selectedMedicalDirectiveList?.map(innerData => format(new Date(innerData?.attestationLog?.lastModifiedDate), 'MMM dd, yyyy')), 'onClickFunction': handleEdit });
        }
        if (medicalDirectivesStatus !== 'completed') {
            // temp.push({
            //     "type": "icon", "icon": selectedMedicalDirectiveList?.map(innerData =>
                ...
                <div>
                    ...
                    <div className={`${style.applicationCardStyle} ${style.marginTop}`}>
                        {!showMedicalDirectives ? allMedicalDirectives?.completed?.length !== undefined && (
                            <>
                                <div className={`${style.cardTitle} ${style.marginTop}`}>{allMedicalDirectives?.pending?.length === 0 ? 'All Medical Directives applicable to me have been reviewed and Signed off.' : 'Medical Directives Review'}</div>
                                <CommonDivider />
                                {/* {allMedicalDirectives?.pending?.length !== 0 && ( */}
                    ...
                    <div className={style.threeColForButton}>
                        ...
                        <Tooltip title={"Click to Save your Progress and Continue later"} arrow>
                            <div className={`${style.saveInProgress} ${style.marginTop}`} onClick={async () => {
                                ...
                                if (showMedicalDirectives) {
                                    if (isSigned) {
                                        handleSubmitAttestBulk();
                                        setShowMedicalDirectives(false);
                                    }
                                } else {
                                    handleContinue("save");
                                }
                        ...
                        <Tooltip title={"Click to Proceed to the Next Step"} arrow>
                            <div className={`${style.continue} ${style.marginTop} ${showMedicalDirectives ? isSigned ? '' : style.disabledButton : ''}`} onClick={showMedicalDirectives ? isSigned ? () => { handleSubmitAttestBulk(); setShowMedicalDirectives(false); } : () => { } : () => handleContinue("continue")}>CONTINUE</div></Tooltip>
                    </div>
                </div>
                <div>
                    {!showInfo && (
                    ...
                    <div className={`${style.stickyContainer} ${isSaveInProgressOpen ? style.hiddenStickyContainer : ""}`}>
                        ...
                        <div className={style.twoColForButton}>
                            ...
                            <Tooltip title={showMedicalDirectives ? isSigned ? "Click to Proceed to the Next Step" : "" : ""} arrow>
                                <div className={`${style.continue} ${style.marginTop10} ${showMedicalDirectives ? isSigned ? '' : style.disabledButton : ''}`} onClick={showMedicalDirectives ? isSigned ? () => { handleSubmitAttestBulk(); setShowMedicalDirectives(false); } : () => { } : () => handleContinue("continue")}>CONTINUE</div></Tooltip>
                        </div>
                    </div>
