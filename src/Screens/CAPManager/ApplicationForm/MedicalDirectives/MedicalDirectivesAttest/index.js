...
const MedicalDirectivesAttestInitialApplication = () => {
    ...
    const getAttestationLog = async () => {
        if (medicalDirectivesId !== undefined && users?.id !== undefined) {
            const { data: medicalDirectivesAttestationLog } = await GET(
                `medical-directive-service/attestationLog?userId=${users?.id}&medicalDirectiveId=${medicalDirectivesId}`
            );
    ...
    return (
        <div className={style.screenBackground}>
            <div className={style.welcomeText}>
                <ApplicationHeader title={`${medicalDirectives?.title}`} close={true} closeClick={handleClose} isNotLogout={true} />
            </div>
