...
const ApplicationAcknowledgementStep3 = ({
  ...
  useEffect(() => {
    ...
    setSignText(
      basicForm?.forms?.[13]?.acknowledged
        ? basicForm?.forms?.[13]?.esign?.esign
        : ""
    );
    setIsSigned(
      basicForm?.forms?.[13]?.esign?.esign !== undefined &&
        basicForm?.forms?.[13]?.acknowledged
        ? true
        : false
    );
    ...
    if (file !== null) {
      ...
      try {
        const response = await PUT(
          `application-management-service/application/${applicationId}/form/${basicForm?.forms?.[13]?.id}/addFileToForm`,
          uploadedFile
        );
        console.log(response?.data);
  ...
  const handleSubmitApplicationReq = async () => {
    if (isSigned) {
      ...
      await PUT(
        `application-management-service/application/${basicForm?.id}/form/${basicForm?.forms?.[13]?.id}`,
        temp
      )
        .then((response) => {
