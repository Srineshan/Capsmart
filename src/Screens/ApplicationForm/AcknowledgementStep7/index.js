...
const ApplicationAcknowledgementStep7 = ({ acknowledgementForm, dateFormat, name, basicForm, getPreApplication, applicationId }) => {
    ...
    useEffect(() => {
        ...
        setIsChecked(basicForm?.forms?.[17]?.acknowledged);
        // setEncryptedText(basicForm?.forms?.[17]?.esign?.esign)
        setSignText(basicForm?.forms?.[17]?.acknowledged ? basicForm?.forms?.[17]?.esign?.esign : '');
        setIsSigned((basicForm?.forms?.[17]?.esign?.esign !== undefined && basicForm?.forms?.[17]?.acknowledged) ? true : false);
        // setDecryptedText(CryptoJS.AES.decrypt(basicForm?.forms?.[17]?.esign?.esign, publicKey).toString(CryptoJS.enc.Utf8))
    }, [basicForm])

    useEffect(() => {
        if (basicForm?.forms?.[17]?.id !== undefined) {
            getRenderedContent()
        }
    }, [basicForm?.forms?.[17]?.id])

    const getFormSchema = async () => {
        if (basicForm?.formSchemas?.[17]?.id !== undefined) {
    ...
    const getRenderedContent = async () => {
        const { data: content } = await GET(
            `application-management-service/application/${basicForm?.id}/form/${basicForm?.forms?.[17]?.id}/render`
        );
    ...
    const addNewDocument = async (file) => {
        ...
        let fileName = {
            "fileName": 'conflictOfInterest.pdf'
        };
        ...
        if (file !== null) {
            ...
            try {
                const response = await PUT(`application-management-service/application/${applicationId}/form/${basicForm?.forms?.[17]?.id}/addFileToForm`, uploadedFile);
                console.log(response?.data);
    ...
    const handleSubmitApplicationReq = async () => {
        if (isSigned) {
            let temp = {
                schemaId: basicForm?.forms?.[17]?.schemaId,
                data: !isEdited ? basicForm?.forms?.[17]?.data : { esignDate: isChecked ? name + " " + currentDate : '' },
                acknowledged: isChecked,
                esign: { esign: isChecked ? encryptedText : '', name: isChecked ? name : '', signedDate: isChecked ? currentDate : '' }
            }
            await PUT(`application-management-service/application/${basicForm?.id}/form/${basicForm?.forms?.[17]?.id}`, temp)
                .then(response => {
                            ...
                            <div className={style.twoCol}>
                                ...
                                <div className={style.verticalAlignCenter}>
                                    <div className={style.displayInRow}>
                                        ...
                                        <div className={`${style.date} ${style.marginLeft}`}>{isSigned ? (basicForm?.forms?.[17]?.esign?.signedDate !== '' && basicForm?.forms?.[17]?.esign?.signedDate !== undefined) ? basicForm?.forms?.[17]?.esign?.signedDate : currentDate : ""}</div>
                                    </div>
                                </div>
                            </div>
                        )}
