...
const ApplicationAcknowledgementStep2 = ({
  ...
  let questionsArray = [
    ...
    "In your professional opinion, please state how the medical condition, disease or illness for which you are treating (or have treated) the Applicant will affect his or her ability to carry out the practice of medicine and, in particular, his or her medical specialty?",
  ];
  useEffect(() => {
    ...
    setSignText(
      basicForm?.forms?.[12]?.acknowledged
        ? basicForm?.forms?.[12]?.esign?.esign
        : ""
    );
    setIsSigned(
      basicForm?.forms?.[12]?.esign?.esign !== undefined &&
        basicForm?.forms?.[12]?.acknowledged
        ? true
        : false
    );
  ...
  const getFormSchema = async () => {
    if (basicForm?.formSchemas?.[12]?.id !== undefined) {
    ...
    if (file !== null) {
      ...
      try {
        const response = await PUT(
          `application-management-service/application/${applicationId}/form/${basicForm?.forms?.[12]?.id}/addFileToForm`,
          uploadedFile
        );
        console.log(response?.data);
  ...
  const handleSubmitApplicationReq = async () => {
    if (isSigned) {
      ...
      await PUT(
        `application-management-service/application/${basicForm?.id}/form/${basicForm?.forms?.[12]?.id}`,
        temp
      )
        .then((response) => {
                            ...
                            <div className={style.twoCol}>
                                ...
                                <div className={style.verticalAlignCenter}>
                                    <div className={style.displayInRow}>
                                        ...
                                        <div className={`${style.date} ${style.marginLeft}`}>{isSigned ? (basicForm?.forms?.[12]?.esign?.signedDate !== '' && basicForm?.forms?.[12]?.esign?.signedDate !== undefined) ? basicForm?.forms?.[12]?.esign?.signedDate : currentDate : ""}</div>
                                    </div>
                                </div>
                            </div>
                        )}
                        <CommonDivider />
                        {questionsArray?.map((data, index) => (
                            <div className={style.marginTop10}>
                                <div className={style.spaceToFill}>{data}</div>
                                <CommonDivider />
                            </div>
                        ))}
                        <div className={`${style.signatureGridManual} ${style.marginTop}`}>
                            <div className={style.dateTitle}>Date:</div>
                            <div className={style.dateTitle}>Signature:</div>
                        </div>
                        <br />
                    </div>
                </div>
