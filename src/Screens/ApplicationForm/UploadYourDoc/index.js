        ...
        <div>
            ...
            <div className={`${style.applicationScreenGrid} ${style.marginTop}`}>
                <div>
                    ...
                    <div className={`${style.descriptionText} ${style.marginTop10}`}>Ensure your documents are in the following formats: pdf, jpg, (add icons here) </div> */}
                        ...
                        {formSchema !== undefined && 'uploadTheDocument' in formSchema?.properties && (
                            <ApplicationFieldCard object={formSchema?.properties?.uploadTheDocument} gridStyle={style.twoCol} baseKey={'uploadTheDocument'} basicForm={basicForm} setBasicForm={setBasicForm} stepPath={`forms[${formIndex}].data`} />
                        )}
                            ...
                            <div className={style.padding20}>
                                ...
                                {isCollapsableCard && (
                                    <>
                                        ...
                                        {basicForm?.documentsRequired?.map((data, index) => (
                                            <div>
                                                <div className={`${style.requiredDocumentCard} ${style.tableGrid} ${(basicForm?.forms?.[formIndex]?.data !== null && tempValue?.table?.filter(tableData => tableData?.documentType === data?.document?.name)?.length === 0 && data?.required) ? style.redBorder : ''} ${index % 2 === 0 ? style.requiredDocumentCardAlternativeColor : ''}  ${style.marginTop5}`}>
                        ...
                        {(basicForm?.forms?.[formIndex]?.data !== null && !showRedBorderForESign) ? (
                            ...
                            <div className={style.marginTop} onClick={() => setIsShowESignDialog(true)}>
                                <div className={`${style.uploadBorderStyle} ${(basicForm?.forms?.[formIndex]?.data !== null && showRedBorderForESign) ? style.redBorder : ''}`}>
                ...
                <div>
                    ...
                    <div className={`${style.saveInProgress} ${style.marginTop10} ${(basicForm?.forms?.[formIndex]?.data !== null && getMissingDocs()?.length === 0) ? style.disabledButton : ''}`} onClick={(basicForm?.forms?.[formIndex]?.data !== null && getMissingDocs()?.length === 0) ? () => { } : () => setIsShowUploadValidation(true)}>SKIP FOR NOW</div>
                    <div className={style.twoColForButton}>
                        <div className={`${style.continue} ${style.marginTop10}`} onClick={() => navigate(-1)}>BACK</div>
                        <div className={`${style.continue} ${style.marginTop10} ${((basicForm?.forms?.[formIndex]?.data !== null && showRedBorderForESign) || (basicForm?.forms?.[formIndex]?.data !== null && getMissingDocs()?.length !== 0)) ? style.disabledButton : ''}`} onClick={((basicForm?.forms?.[formIndex]?.data !== null && showRedBorderForESign) || (basicForm?.forms?.[formIndex]?.data !== null && getMissingDocs()?.length !== 0)) ? () => { } : () => handleContinue()}>CONTINUE</div>
                    </div>
                ...
                isShowESignDialog && (
                    <ESignDialog getIsOpen={getIsOpen} tempValue={tempValue} baseKey={'setUpYourSignature'} applicationId={applicationId} basicForm={basicForm} setBasicForm={setBasicForm}>
                        {formSchema !== undefined && 'setUpYourSignature' in formSchema?.properties && (
                            <ApplicationFieldCard object={formSchema?.properties?.setUpYourSignature} gridStyle={style.twoCol} baseKey={'setUpYourSignature'} basicForm={basicForm} setBasicForm={setBasicForm} stepPath={`forms[${formIndex}].data`} setIsEdited={getIsEdited} />
                        )}
                    </ESignDialog>
                )
            }
