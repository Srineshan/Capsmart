...
const Step10 = ({
  ...
  console.log("basicForm10", basicForm);
  useEffect(() => {
    if (isAddMore) {
      setIsAddMore2(false);
      setMetadata([]);
  ...
  const handleSubmitApplicationReq = async (data) => {
    ...
    await POST(
      `application-management-service/application/validateForm`,
      tempValidation
    )
      .then((response) => {
        console.log(response, response?.response?.data, "missingFields");
        missingFields =
          response?.data !== undefined && response?.data === true
            ? []
    ...
    await PUT(
      `application-management-service/application/${applicationId}/form/${basicForm?.forms?.[7]?.id}`,
      temp
    )
      .then((response) => {
            ...
            {formSchema !== undefined &&
              "privilegeReferences" in formSchema?.properties && (
                <ApplicationFieldCard
                  ...
                  warningFields={warningFields}
                  getMissingFields={getMissingFields}
                  showValidationDialog={showValidationDialog2}
                  setShowValidationDialog={setShowValidationDialog2}
                  isAddMore={isAddMore2}
                  setIsAddMore={setIsAddMore2}
                  formSchema={formSchemaWholeObject}
                />
              )}
          </div>
