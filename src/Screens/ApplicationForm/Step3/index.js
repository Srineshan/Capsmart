...
const Step3 = ({
  ...
  const getMissingFields = () => {
    ...
    metadata?.map((data, index) => {
      keyValuePair.push({
        ...
        label: uniqueLabels?.filter((labelData) => labelData?.path === data)[0]
          ?.label,
      });
    });
    ...
    keyValuePair?.map((data) => {
      ...
      if (
        ...
        data?.value === 0 ||
        (data?.key === "forms[1].data.contactAddress3.business.businessPhone" &&
          !validateBusinessPhone(data?.value)) ||
        (data?.key ===
          "forms[1].data.contactAddress3.business.businessWebsite" &&
          !validateBusinessWebsite(data?.value))
      ) {
        if (
          data?.key ===
            "forms[1].data.contactAddress3.business.businessPhone" &&
          !validateBusinessPhone(data?.value)
        ) {
          setBasicForm((prevForm) => ({
            ...prevForm,
            forms: prevForm.forms.map((form) => {
              if (form.schemaId === basicForm.forms[1].schemaId) {
                return {
                  ...form,
                  data: {
                    ...form.data,
    ...
    if (
      ...
      let registeredBusinessAddressKeys = [
        "forms[1].data.contactAddress3.business.businessName",
        "forms[1].data.contactAddress3.business.businessAddress.streetName",
        "forms[1].data.contactAddress3.business.businessAddress.pinCode",
        "forms[1].data.contactAddress3.business.businessAddress.city",
        "forms[1].data.contactAddress3.business.businessAddress.province",
        "forms[1].data.contactAddress3.business.businessPhone",
        "forms[1].data.contactAddress3.business.businessWebsite",
      ];
      let temp = missingKeys?.filter(
        (data) => !registeredBusinessAddressKeys?.includes(data?.key)
      );
      missingKeys = temp;
    }
  ...
  const handleSubmitApplicationReq = async (skip) => {
    if (isEdited) {
      ...
      await PUT(
        `application-management-service/application/${applicationId}/form/${basicForm?.forms?.[1]?.id}`,
        temp
      )
        .then((response) => {
      ...
      addressData.contactAddress2 =
        basicForm?.forms?.[1]?.data.contactAddress2 !== undefined
          ? basicForm?.forms?.[1]?.data.contactAddress2
          : null;
      addressData.contactAddress3 =
        basicForm?.forms?.[1]?.data.contactAddress3 !== undefined
          ? basicForm?.forms?.[1]?.data.contactAddress3
          : null;
      await PUT(
        `application-management-service/application/${applicationId}/profile`,
        addressData
