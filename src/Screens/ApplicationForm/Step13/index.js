...
const Step13 = ({
  ...
  const getMissingFields = () => {
    ...
    const cellPhone = getValueByPath(
      basicForm,
      "forms[10].data.impactingPractice.medicalHistory.cellPhone"
    );
    const emailId = getValueByPath(
      basicForm,
      "forms[10].data.impactingPractice.medicalHistory.emailId"
    );
    ...
    if (isCellPhoneInvalid) {
      setBasicForm((prevForm) => ({
        ...
        forms: prevForm.forms.map((form) => {
          if (form.schemaId === basicForm.forms[10].schemaId) {
            return {
              ...form,
              data: {
                ...form.data,
  ...
  const handleSubmitApplicationReq = async (data) => {
    if (isEdited) {
      ...
      await PUT(
        `application-management-service/application/${applicationId}/form/${basicForm?.forms?.[10]?.id}`,
        temp
      )
        .then((response) => {
  ...
  const handleSubmitApplication = async () => {
    await POST(
      `application-management-service/application/${applicationId}/submit`
    )
      .then((response) => {
        console.log(response);
            ...
            {formSchema !== undefined &&
              "impactingPractice" in formSchema?.properties && (
                <ApplicationFieldCard
                  ...
                  gridStyle={style.criminalHistoryGrid}
