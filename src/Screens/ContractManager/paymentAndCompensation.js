...
const PaymentAndCompensation = ({
  ...
  const getContractDetail = async () => {
    ...
    setContractPeriod({
      ...contractPeriod,
      start: contractData?.contractDetail?.contractTerm?.effectiveDate,
      end: contractData?.contractDetail?.contractTerm?.endDate,
    });
  ...
  const saveInProgresscheck = () => {
    ...
    if (
      !dollarRate?.notApplicable &&
      valueCheck(dollarRate?.hour?.toLocaleString())
    ) {
  ...
  useEffect(() => {
    ...
    setDollarRate({
      ...dollarRate,
      hour: paymentAndCompensation?.dollarRate?.hour,
      notApplicable: paymentAndCompensation?.dollarRate?.notApplicable,
    });
    setDollarValue(paymentAndCompensation?.dollarValue);
    setCompensationOffsetCriteria(
      paymentAndCompensation?.compensationOffsetCriteria
    );
  ...
  const setTimesheetPaymentsValue = () => {
    ...
    timeSheetTabs?.map((data, index) => {
      let reducedNumberOfServices =
        compensationPolicy === "ACTIVITY_BASED" ||
        compensationPolicy ===
          "FIXED_AMOUNT_FOR_TIMESHEET_PERIOD_WITHOUT_OFFSET"
          ? "NA"
          : timesheetPayments?.[index]?.reducedNumberOfServices || "NA";
      let maxPaymentPerTimesheetSubmission =
        compensationPolicy === "ACTIVITY_BASED"
          ? parseFloat(0)
          : timesheetPayments?.[index]?.maxPaymentPerTimesheetSubmission ||
            parseFloat(0);
      let providingAdditionalServices =
        compensationPolicy === "ACTIVITY_BASED" ||
        compensationPolicy ===
          "FIXED_AMOUNT_FOR_TIMESHEET_PERIOD_WITHOUT_OFFSET"
          ? "NA"
          : timesheetPayments?.[index]?.providingAdditionalServices || "NA";
      temp.push({
        timesheetLabel: {
          label: timeSheetTabs?.[index]?.timesheetLabel?.label,
        },
        paymentFrequency: data?.servicePeriod?.value,
        maxPaymentPerTimesheetSubmission: parseFloat(
          maxPaymentPerTimesheetSubmission
        ),
        maxPaymentPerContract:
          parseFloat(timesheetPayments?.[index]?.maxPaymentPerContract) ||
          parseFloat(0),
        reducedNumberOfServices: reducedNumberOfServices,
        providingAdditionalServices: providingAdditionalServices,
        paymentBasedonFixedHoursVsActual:
          timesheetPayments?.[index]?.paymentBasedonFixedHoursVsActual || true,
      });
    });
            ...
            <>
              {compensationPolicy !== "ACTIVITY_BASED" && (
                <>
                  ...
                  >
                    <CommonLabel
                      value="Fixed Compensation Value Per Timesheet Submission*"
                      className={
                        dataCheck(
                          Number(
                            timesheetPayments?.[i]
                    ...
                    <CommonTextField
                      ...
                      onChange={(e) =>
                        fixedCompensationValue(
                          e.target.value.slice(0, limit9).replace(/,/g, ""),
                          "maxPaymentPerTimesheetSubmission",
                          i
                        )
                      }
                      value={Number(
                        timesheetPayments?.[i]
                          ?.maxPaymentPerTimesheetSubmission || 0
                      )?.toLocaleString()}
                    />
                  </div>
                  {compensationPolicy !==
              ...
              <div className={`${style.extentionGrid} ${style.marginTop20}`}>
                <CommonLabel
                  value="Max. Compensation Value for Contract Period*"
                  className={
                    dataCheck(
                      Number(timesheetPayments?.[i]?.maxPaymentPerContract)
                    )
                      ? style.redLable
                      : ""
                  }
                ...
                <div className={style.displayInRow}>
                  <CommonTextField
                    ...
                    value={(
                      timesheetPayments?.[i]?.maxPaymentPerContract || 0
                    )?.toLocaleString()}
                  ...
                  <CommonLabel
                    ...
                    value={`$ ${(
                      timesheetPayments?.[i]?.maxPaymentPerContract /
                        (monthDiff(
                          new Date(contractPeriod?.start),
                          new Date(contractPeriod?.end)
                        ) /
                          12) || 0
                    )?.toLocaleString()} Per Contract Year`}
                  />
                </div>
              </div>
              {compensationPolicy !== "ACTIVITY_BASED" &&
                compensationPolicy !==
                  ...
                  >
                    ...
                    <CommonSelectField
                      value={
                        timesheetPayments?.[i]?.providingAdditionalServices
                      }
                      onChange={(e) =>
                        updateTimesheetPayment(
                          e.target.value,
  ...
  const leftElement = () => {
    return <button className={`${style.dollarLeftElement}`}>$</button>;
  };
    ...
    <>
      {timeSheetTabs?.length !== 0 ? (
        <div className={style.cloneBlockStyle}>
          <div className={`${style.newContractFromCloneBoxStyle}`}>
            <div
              ...
              onFocus={() => {
                checkFieldAndPopAlert(compensation, "Compensation Basis");
              }}
            >
              ...
              <div>
                ...
                >
                  <CommonLabel
                    value="RVU Quantity*"
                    className={
                      dataCheck(rvuQuantity?.quantity) ? style.redLable : ""
                    }
                  />
                  <div className={style.displayInRow}>
                    <CommonInputField
                      className={style.fourFieldWidth}
                      value={rvuQuantity?.quantity}
                      placeholder="0"
                      type="number"
                      min="0"
                      onChange={(e) =>
                        ...
                        setRvuQuantity({
                          ...rvuQuantity,
                          quantity: e.target.value.slice(0, limit5),
                        })
                      }
                    />
                    <CommonSelectField
                ...
                >
                  <CommonLabel
                    ...
                    className={
                      dataCheck(fteEquivalent?.value) ? style.redLable : ""
                    }
                  />
                  <CommonInputField
                    className={style.twoFieldWidth}
                    ...
                    onChange={(e) =>
                      setFteEquivalent({
                        ...fteEquivalent,
                        value: parseFloat(e.target.value.slice(0, limit4)),
                      })
                    }
                  />
                </div>
                <div
                  ...
                  onFocus={() => {
                    checkFieldAndPopAlert(
                      rvuReferenceUsed?.value,
                      "RVU Reference Used"
                    );
                  }}
                >
                  <CommonLabel
                    value="RVU Reference Used"
                    className={
                      dataCheck(rvuReferenceUsed?.value) ? style.redLable : ""
                    }
                  />
                  <CommonInputField
                    className={style.fullWidth}
                    value={rvuReferenceUsed?.value}
                    placeholder="Enter RVU Reference Used"
                    min="0"
                    onChange={(e) =>
                      setRvuReferenceUsed({
                        ...rvuReferenceUsed,
                ...
                <div
                  ...
                  onFocus={() => {
                    checkFieldAndPopAlert(
                      rvuQuantityVariance?.value,
                      "RVU Quantity Variance (+/-)"
                    );
                  }}
                >
                  <CommonLabel
                    value="RVU Quantity Variance (+/-)"
                    className={
                      dataCheck(rvuQuantityVariance?.value)
                        ? style.redLable
                        : ""
                    }
                  />
                  <CommonInputField
                    ...
                    min="0"
                    onChange={(e) =>
                      setRvuQuantityVariance({
                        ...rvuQuantityVariance,
                        value: e.target.value.slice(0, limit3),
                      })
                    }
                  />
                </div>
                <div className={`${style.extentionGrid} ${style.marginTop20}`}>
                  <CommonLabel
                    value="RVU Quantity Period"
                    className={
                      dataCheck(rvuQuantityPeriod?.days) ? style.redLable : ""
                    }
                  />
                  <CommonTextField
                    InputProps={{
                    ...
                    onChange={(e) =>
                      setRvuQuantityPeriod({
                        ...rvuQuantityPeriod,
                        days: e.target.value.slice(0, limit4),
                      })
                    }
            ...
            <div className={`${style.extentionGrid} ${style.marginTop20}`}>
              <CommonLabel
                value="Dollar Hourly Rate*"
                className={
                  !dollarRate?.notApplicable &&
                  dataCheck(dollarRate?.hour?.toLocaleString())
                    ? style.redLable
                    : ""
                }
              />
              <div className={style.twoCol}>
                <CommonTextField
                  ...
                  onChange={(e) =>
                    setDollarRate({
                      ...dollarRate,
                      hour: e.target.value.slice(0, limit9),
                      notApplicable: false,
                    })
                  }
                />
                <CommonCheckBox
                  value="NA"
                  checked={dollarRate?.notApplicable}
                  onChange={(e) =>
                    setDollarRate({
                      ...dollarRate,
            ...
            <div
              className={`${style.paymentTimesheetDetailsHeading} ${style.marginTop20}`}
            >
          ...
          {isEditable && (
            <div className={`${style.spaceBetween} ${style.marginTop20}`}>
              <button
                ...
                onClick={() => {
                  getCurrentPage("Timesheet Submission Terms");
                }}
              >
                BACK
              </button>
              <div>
                ...
                <button
                  ...
                  onClick={() => mandatoryFieldCheck("Continue")}
                >
                  CONTINUE
                </button>
              </div>
        ...
        <RedirectingPopUp
          ...
          tabName={"Timesheet Submission Terms"}
          title={"NO TIMESHEET FOUND"}
          description={"No Timesheet Is Found."}
