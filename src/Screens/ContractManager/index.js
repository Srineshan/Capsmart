...
import ActiveDraftContract from "./activeDraftContract";
...
const Contracts = () => {
  ...
  useEffect(() => {
    sessionStorage.setItem(
      "isEditable",
      selectedContract === "draft" ? true : false
    );
  }, [selectedContract]);

  useEffect(() => {
    setIsEditable(
      sessionStorage.getItem("isEditable") === "true" ? true : false
    );
  ...
  const getActiveDraftDialog = (value) => {
    setActiveDraftDialog(value);
  };
  ...
  const getActivationAlertDraftDialog = (value) => {
  ...
  const getContracts = async () => {
    const { data: contracts } = await GET(
      `contract-managment-service/contracts?limit=${10}&offset=${
        page - 1
      }&searchText=${searchKey}&tab=${selectedContract}`
    );
  ...
  return addContract ? (
    ...
    <ActiveContract
      ...
      activeContractView={activeContractView}
    ...
    <Fragment>
      ...
      {activeDraftDialog && (
        <ActiveDraftContract getActiveDraftDialog={getActiveDraftDialog} />
      )}
