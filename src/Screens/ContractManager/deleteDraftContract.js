        ...
        <Dialog isOpen={getDeleteDraftDialog} onClose={() => getDeleteDraftDialog(false)} className={`${style.dialogStyle} ${style.dialogPaddingBottom}`}>
            <div className={`${Classes.DIALOG_BODY} ${style.extensionDialogBackground}`}>
                ...
                <div className={`${style.padding20} ${style.marginTop20}`}>
                    <TextArea placeholder='NOTES For Deleting THIS CONTRACT DRAFT' rows="4" className={style.fullWidth} />
                </div>
                {/* </div>
            </div> */}

                <p className={`${style.cloneContent} ${style.marginTop20}`}>NOTE THAT ONCE YOU DELETE THIS DRAFT CONTRACT, IT WILL NO LONGER BE AVAILABLE FOR YOU TO WORK WITH. ALL THE INFORMATION ENTERED IN THIS CONTRACT WILL BE DELETED.</p>
                <div>
