...
const DeleteDraftContract = ({ getDeleteDraftDialog }) => {
  return (
    <Dialog
      isOpen={getDeleteDraftDialog}
      onClose={() => getDeleteDraftDialog(false)}
      ...
      >
        <div className={style.spaceBetween}>
          ...
          <Icon
            ...
            onClick={() => getDeleteDraftDialog(false)}
          />
        </div>
        ...
        <div className={`${style.padding20} ${style.marginTop20}`}>
          <TextArea
            placeholder="NOTES For Deleting THIS CONTRACT DRAFT"
            rows="4"
        ...
        <p className={`${style.cloneContent} ${style.marginTop20}`}>
          NOTE THAT ONCE YOU DELETE THIS DRAFT CONTRACT, IT WILL NO LONGER BE
          AVAILABLE FOR YOU TO WORK WITH. ALL THE INFORMATION ENTERED IN THIS
