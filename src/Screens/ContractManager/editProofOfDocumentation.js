...
const EditProofOfDocumentation = ({getShowEditProofDialog, isMultipleContract, contractId, selectedProof}) => {
    ...
    useEffect(() => {
        setSelectedPOD(selectedProof?.podType?.type);
        setContractorName(selectedProof?.dataMap?.dataMap?.contractor);
        setSpecialityBoardName(selectedProof?.dataMap?.dataMap?.specialityBoard);
        setMedicalStaffId(selectedProof?.dataMap?.dataMap?.medicalStaffId);
        setSpecialityBoardCertificateId(selectedProof?.dataMap?.dataMap?.specialityBoardCertificateId);
        setInsuranceCarrierName(selectedProof?.dataMap?.dataMap?.insuranceCarrierName);
        setInsuranceCertificateId(selectedProof?.dataMap?.dataMap?.insuranceCertificateId);
        setStateOfLicensure(selectedProof?.dataMap?.dataMap?.stateOfLicensure);
        setLicenseId(selectedProof?.dataMap?.dataMap?.licenseId);
        setCertificateId(selectedProof?.dataMap?.dataMap?.certificateId);
        setmembershipRenewalDate(selectedProof?.expirationDate?.date !== null ? new Date(selectedProof?.expirationDate?.date) : null);
        setFileName(selectedProof?.file?.fileName);
        setFilePath(selectedProof?.file?.filePath);
        setFileData(null);
        setSelectedSite(selectedProof?.dataMap?.dataMap?.privilegingFacility);
        setCertificateCopyAvbl(selectedProof?.certificateCopyAvailable);
        setSelectedInsuranceCarrier(selectedProof?.dataMap?.dataMap?.coverageToBeProvidedBy);

    }, [])

    console.log('Membership Renewal Date',membershipRenewalDate,new Date(membershipRenewalDate));
    ...
    const handleContinue = async () => {
      ...
      if(!isMultipleContract){
         ...
         data = selectedPOD === 'Medical Staff Membership & Privileges' ?
        {
          ...
          expirationDate: {date:membershipRenewalDate !== null ? format(new Date(membershipRenewalDate), 'yyyy-MM-dd').toString() : null},
          file: {
          filePath: filePath,
          fileName: fileName
            },
      ...
      if(data?.certificateCopyAvailable && fileData === null && fileName === ''){
      ...
      const formData = new FormData();
       ...
       await PUT(`contract-managment-service/contracts/${contractId}/DocumentationProof`, formData)
       .then(response=>{
    ...
    return(
        <Dialog isOpen={getShowEditProofDialog} onClose={() => getShowEditProofDialog(false)} className={`${style.addManagerDialogBackground} ${style.addProofDialog}`}>
          <div className={`${Classes.DIALOG_BODY} `}>
            <div className={style.spaceBetween}>
                <p className={style.extensionStyle}>Edit Proof Of Documentation</p>
                <Icon icon="cross" size={20} intent={Intent.DANGER} className={style.crossStyle} onClick={() => getShowEditProofDialog(false)}  />
            </div>
                  ...
                  <div className={`${style.addManagerGrid} ${style.marginTop20}`}>
                   ...
                   <div className={`${style.addManagerGrid} ${style.marginTop20}`}>
                     ...
                     <select
                      name="class"
                      id="Class"
                      value={selectedSite?.siteName?.siteName}
            ...
            <div className={`${style.addManagerGrid} ${style.marginTop20}`}>
              ...
              <div className={`${style.floatRight} ${style.marginTop20}`}>
                  <button className={`${style.buttonStyle} ${style.marginLeft20}`} onClick={() => {handleContinue(); getShowEditProofDialog(false)}}>UPDATE</button>
              </div>
            </div>
        </Dialog>
    )
...
export default EditProofOfDocumentation;
