...
const EditServiceProvider = ({
  ...
  useEffect(() => {
    setNpin({
      npin: userProviderData?.npin?.npin,
    ...
    setUserDetails({
      ...
      middleName: userProviderData?.name?.middleName || "",
      lastName: userProviderData?.name?.lastName || "",
      suffix: {
        suffix: userProviderData?.name?.suffix?.suffix || "",
        id: userProviderData?.name?.suffix?.id,
      },
      email: userProviderData?.email?.officialEmail || "",
      phone: userProviderData?.communication?.mobileNumber || "",
    ...
    setAddress({
      addressLine: userProviderData?.address?.addressLine || "",
      city: userProviderData?.address?.city || "",
  ...
  const leftElement = () => {
    return <Button text="Upload" intent={Intent.PRIMARY} />;
  };
  ...
  const getContractWorkFlowUser = async () => {
    const { data: contractWorkflow } = await GET(
      `contract-managment-service/contracts/workFlowUser`
    );
    if (contractWorkflow) {
  ...
  const rolesTags = selectedRoles
    ...
    ?.map((tag, index) => {
      const onRemove = () => {
        setSelectedRoles(
          selectedRoles.filter((t) => t?.roleName !== tag?.roleName)
        );
      };
      return (
        <Tag
  ...
  const getRolesData = async () => {
    const { data: roles } = await GET(
      `user-management-service/roles?roleType=APP&roleType=APP_SYSTEM`
    );
    if (roles) {
      setRoles(roles);
    }
  ...
  return (
    <Dialog
      ...
      onClose={() => getEditServiceDialog(false)}
      ...
      >
        <div className={style.spaceBetween}>
          ...
          <Icon
            ...
            onClick={() => getEditServiceDialog(false)}
          />
        </div>
          ...
          <div className={`${style.extentionGrid}`}>
            ...
            <div className={style.grid3}>
              ...
              <CommonInputField
                ...
                placeholder="Middle"
                onChange={(e) => handleUserData("middleName", e.target.value)}
              />
              <CommonInputField
                className={style.fullWidth}
          ...
          <div className={`${style.extentionGrid} ${style.marginTop20}`}>
            ...
            <div className={style.grid3}>
              <CommonInputField
                ...
                value={npin?.npin}
                onChange={(e) =>
                  setNpin({ npin: e.target.value, na: false, missing: false })
                }
              />
              <CommonCheckBox
                value={npin?.missing}
                checked={npin?.missing}
                onChange={(e) =>
                  setNpin({ npin: "", missing: e.target.checked, na: false })
                }
                label="Missing"
              />
              <CommonCheckBox
                ...
                checked={npin?.na}
                onChange={(e) =>
                  setNpin({ npin: "", missing: false, na: e.target.checked })
                }
          ...
          <div className={`${style.extentionGrid} ${style.marginTop20}`}>
            ...
            <div className={style.grid3}>
              <SuffixList
                ...
                onChangeFunc={(id, value) => handleSuffixChange(id, value)}
                className={[style.fullWidth]}
              />
            </div>
          </div>
            ...
            <div className={style.twoCol}>
              ...
              >
                ...
                <CommonInputField
                  ...
                  disabled={phoneNA}
                  onChange={(e) =>
                    handleUserData("phone", FormatPhoneNumber(e.target.value))
                  }
                />
              </div>
              <CommonCheckBox
                ...
                checked={phoneNA}
                onChange={(e) => {
                  setPhoneNA(e.target.checked);
                  if (e.target.checked) {
                    handleUserData("phone", "");
                  }
                }}
                label="NA"
              />
