...
const AddFeedbackTicket = ({getAddFeedbackTicketDialog}) => {
    ...
    return(
        <Dialog isOpen={getAddFeedbackTicketDialog} onClose={() => getAddFeedbackTicketDialog(false)} className={`${style.dialogStyle} ${style.dialogPaddingBottom}`}>
          <div className={`${Classes.DIALOG_BODY} ${style.extensionDialogBackground}`}>
            <div className={style.spaceBetween}>
                <p className={style.extensionStyle}>Add Feedback Ticket</p>
                <Icon icon="cross" size={20} intent={Intent.DANGER} className={style.crossStyle} onClick={() => getAddFeedbackTicketDialog(false)}  />
            </div>
...
export default AddFeedbackTicket;