                            ...
                            <div className={style.scrollStyle}>
                                {standardTemplates?.map((data, index) => (
                                    <div className={`${style.reportsTableGrid} ${style.marginTop20}`} key={index}>
                                        ...
                                        <div className={style.tableDataReportsFontStyle}>{data?.lastRun !== null ? formatInTimeZone(new Date(data?.lastRun), siteTimeZone !== 'undefined' ? siteTimeZone : userTimeZone, 'd MMM yyyy HH:mm') : '-'} </div>
