...
export const Run = ({ link }) => {
  ...
  return (
    ...
    >
      <div
        className={`${style.reportStyle} ${style.blueCard} ${style.cursorPointer}`}
      >
        Run
      </div>
      <Popover
...
const TimeSheetReports = ({ getShowSampleReport }) => {
  ...
  const routeList = {
    ...
    SITE_DEPARTMENT_SPECIFIC_CONTRACTOR_SUMMARY:
      "siteDepartmentSpecificContractorSummary",
  ...
  const descriptionList = {
    ...
    ADDON_ACTIVITES_SERVICES_LOG_SUMMARY:
      "Add On Activities/ Services Requests Status Summary",
    ...
    COST_REPORT_FOR_CONTRACTED_SERVICES_PERFORMED:
      "Cost Report for Contracted Services Performed",
    TIME_AND_PAYEMENT_LOG_FOR_CONTRACTED_SERVICES:
      "Time and Payment Log for Contracted Services",
    SITE_DEPARTMENT_SPECIFIC_CONTRACTOR_SUMMARY:
      "Site/ Department Specific Contractor Summary Statistics",
  ...
  const showMyReport = (data) => {
    ...
    sessionStorage.setItem(
      "reportFilter",
      JSON.stringify(data?.report?.filters)
    );
  ...
  const getMyReports = async () => {
    const { data: myReport } = await GET(
      `timesheet-management-service/report/myReport?userId=${currentUserDetails?.id}&category=${availableCategories[reportType]}`
    );
    setMyReports(myReport);
  };

  const getSavedReports = async () => {
    const { data: savedReport } = await GET(
      `timesheet-management-service/report/savedReport?userId=${currentUserDetails?.id}&category=${availableCategories[reportType]}`
    );
    setSavedReports(savedReport);
  };

  const getStandardTemplates = async () => {
    const { data: standardTemplates } = await GET(
      `timesheet-management-service/report/standardTemplates?userId=${currentUserDetails?.id}&category=${availableCategories[reportType]}`
    );
    ...
    <div className={style.margin20}>
      <div className={isExpanded ? style.bigCardGrid : style.smallCardGrid}>
        ...
        <div className={style.bigCardStyle}>
          <div className={style.paginationCol}>
            <div className={` ${style.titleStyle} ${style.margin20}`}>
              {reportType === "servicesOrActivities"
                ...
                ? "Timesheets"
                : reportType === "reviewsAndApprovals"
                ? "Reviews & Approval"
                : reportType === "taskManagement"
                ? "Task Management"
                : reportType === "payments"
                ? "Payments"
                ...
                : reportType === "systemAdministration"
          ...
          <div
            className={`${style.reportsMargin20} ${style.spaceBetween} ${style.borderBottomReportsStyle}`}
          >
            <div
              className={`${style.reportsTabWidth} ${
                tabName === "Standard Report Templates" &&
                style.selectedReportsTab
              }`}
            >
              <div className={style.spaceBetween}>
                <div
                  className={`${style.displayInRow} ${style.cursorPointer}`}
                  onClick={() => setTabName("Standard Report Templates")}
                >
                  <img
                    src={TemplateIcon}
                    alt="template"
            ...
            <div
              className={`${style.reportsTabWidth} ${
                tabName === "My Reports" && style.selectedReportsTab
              } `}
            >
              <div className={style.spaceBetween}>
                <div
                  className={`${style.displayInRow} ${style.cursorPointer}`}
                  onClick={() => setTabName("My Reports")}
                >
                  <img src={Reject} alt="reject" className={style.iconStyle} />
                  <p
                    className={`${style.taskFontStyle} ${
                      tabName === "My Reports" && style.selectedStyle
                    }`}
                  >
                    My Reports
                  </p>
            ...
            <div
              className={`${style.reportsTabWidth} ${
                tabName === "Saved Report Outputs" && style.selectedReportsTab
              }`}
            >
              <div className={style.spaceBetween}>
                <div
                  className={`${style.displayInRow} ${style.cursorPointer}`}
                  onClick={() => setTabName("Saved Report Outputs")}
                >
                  <img src={Reject} alt="req" className={style.iconStyle} />
                  <p
                    className={`${style.taskFontStyle} ${
                      tabName === "Saved Report Outputs" && style.selectedStyle
                    }`}
                  >
            ...
            <div className={`${style.marginLeft20} ${style.marginTop20}`}>
              ...
              <div className={style.scrollStyle}>
                {standardTemplates?.map((data, index) => (
                  <div
                    className={`${style.reportsTableGrid} ${style.marginTop20}`}
                    key={index}
                  >
                    ...
                    <Link
                      to={`/reportTypeOverview/${routeList[data?.subCategory]}`}
              ...
              <div className={style.scrollStyle}>
                {myReports?.length !== 0 ? (
                  myReports?.map((data, index) => (
                    <div
                      className={`${style.timeSheetTableGrid} ${style.marginTop20}`}
                      key={index}
                    >
                      <div className={style.tableDataReportsFontStyle}>
                        {index + 1}
                      ...
                      <div className={style.tableDataReportsFontStyle}>
                        {formatInTimeZone(
                          new Date(data?.report?.lastUpdated),
                          "America/New_York",
                          "d MMM yyyy"
                        )}{" "}
                      </div>
                      <div className={style.tableDataReportsFontStyle}>
                        {getScheduleValue(data?.report?.schedule?.schedule)}
                      </div>
                      <div
                        className={style.tableDataReportsFontStyle}
                      >{`${data?.report?.owner?.name?.firstName} ${data?.report?.owner?.name?.lastName}`}</div>
                      <div className={style.tableDataReportsFontStyle}>
                        {formatInTimeZone(
                          new Date(data?.report?.lastUpdated),
                          "America/New_York",
                  ...
                  <ReportNoDataBox
                    heading={"There are no scheduled reports for you to run."}
              ...
              <div className={style.scrollStyle}>
                {savedReports?.length !== 0 ? (
                  savedReports?.map((data, index) => (
                    <div
                      className={`${style.savedReportTableGrid} ${style.marginTop20}`}
                    >
                      <div className={style.tableDataReportsFontStyle}>
                        {index + 1}
                      </div>
                      <div className={style.tableDataReportsFontStyle}>
                        {data?.savedReport?.reportName}
                      </div>
                      <div className={style.tableDataReportsFontStyle}>
                        {data?.savedReport?.reportNotes}
                      </div>
                      <div className={style.tableDataReportsFontStyle}>
                        {formatInTimeZone(
                          new Date(data?.savedReport?.runDate),
                          "America/New_York",
                          "d MMM yyyy"
                        )}
                      </div>
                      <div
                        className={`${style.reportStyle} ${style.redCard} ${style.cursorPointer}`}
                      >
                        Delete
                      </div>
                    </div>
                  ...
                  <ReportNoDataBox
                    heading={"There are no Saved reports for you to run."}
