...
const ReportTypeOverview = () => {
  ...
  const [activitiesOrServicesValues, setActivitiesOrServicesValues] =
    useState();
  ...
  const [rejectedTimesheetCountBreakUp, setRejectedTimesheetCountBreakUp] =
    useState();
  const [totalTimesheetRejectedCount, setTotalTimesheetRejectedCount] =
    useState(0);
  ...
  const [timesheetProcessingSummaryData, setTimesheetProcessingSummaryData] =
    useState();
  const [
    submittedTimesheetsPaymentStatusData,
    setSubmittedTimesheetsPaymentStatusData,
  ] = useState();
  const [isNonCompliantReportTileClicked, setIsNonCompliantReportTileClicked] =
    useState(false);
  ...
  useEffect(() => {
    setIndividualContract(
      contractRenewalReport
        ?.filter((data) => data?.contractType === "INDIVIDUAL")
        ?.map((data) => data)
    );
    setMultipleContract(
      contractRenewalReport
        ?.filter((data) => data?.contractType === "MULTIPLE")
        ?.map((data) => data)
    );
  }, [contractRenewalReport]);
  ...
  const reportTitleList = {
    ...
    oneTimeContract:
      "List of One Time Contracts that will Terminate on Expiration",
    scheduledActivity: "Scheduled Activity/ Services - Forcasted To Actual",
    scheduledActivityByContract:
      "Scheduled Activity/ Services - Forcasted To Actual By Contract",
    complianceStatus: "Proof Of Documentation Status By Contractor",
    nonCompliant:
      "List of Contracts that are non compliant with proof of documentation requirement",
  ...
  const getAcvityAndServices = async () => {
    if (!isMyReport) {
      const { data: chartDataValues } = await GET(
        `timesheet-management-service/report/activityServiceReport?startDate=${dataToUseInReport?.from}&endDate=${dataToUseInReport?.to}&users=${dataToUseInReport?.selectedContractedServiceProvider}`
      );
      setActivitiesOrServicesValues(chartDataValues);
    } else {
      const { data: chartDataValues } = await GET(
        `timesheet-management-service/report/myReport/activityServiceReport?id=${myReportId}`
      );
      setActivitiesOrServicesValues(chartDataValues);
    }
  };
  ...
  const setActivitiesOrServices = () => {
    ...
    if (activitiesOrServicesValues) {
      ...
      setPieData(temp);
      let lineTemp = [];
      activitiesOrServicesValues?.completedActivitiesByDate?.map(
        (line, index) => {
          lineTemp[index] = {
            date: format(new Date(line.statedate), "MM-dd"),
            value: line.count,
          };
        }
      );
      ...
      if (activitiesOrServicesValues?.activityStatusByCategorys?.length !== 0) {
        setSeries([
          {
            data: activitiesOrServicesValues?.activityStatusByCategorys?.map(
              (data) => data?.done
            ),
            name: "Done",
          },
        ...
        setCategories(
          activitiesOrServicesValues?.activityStatusByCategorys?.map(
      ...
      let stackedTemp = [];
      let keysForChart = [];
      activitiesOrServicesValues?.completedActivitiesBycategoryAndMonth?.map(
        (stack, index) => {
      ...
      keysForChart?.map((data, index) => {
        tempStackedSeries.push({
          data: stackedTemp?.map((stackedData) =>
            stackedData?.[data] ? stackedData?.[data] : 0
          ),
          name: data,
        });
      });
      setStackedSeries(tempStackedSeries);
      setStackedCategories(
        stackedTemp?.map((stackedData) => stackedData?.name)
      );
    }
    setIsLoading(false);
  ...
  const getAddOnServices = async () => {
    if (!isMyReport) {
      const { data: chartDataValues } = await GET(
        `timesheet-management-service/report/addOnActivityServiceReport?startDate=${dataToUseInReport?.from}&endDate=${dataToUseInReport?.to}&users=${dataToUseInReport?.selectedContractedServiceProvider}`
      );
      setAddOnServicesValues(chartDataValues);
    } else {
      const { data: chartDataValues } = await GET(
        `timesheet-management-service/report/myReport/addOnActivityServiceReport?id=${myReportId}`
      );
      setAddOnServicesValues(chartDataValues);
    }
  };
  ...
  const setAddOnServices = () => {
    ...
    if (addOnServicesValues) {
      ...
      if (addOnServicesValues?.addOnActivityStatusByCategorys?.length !== 0) {
        setSeries([
          {
            data: addOnServicesValues?.addOnActivityStatusByCategorys?.map(
              (data) => data?.approved
            ),
            name: "Approved",
          },
        ...
        setCategories(
          addOnServicesValues?.addOnActivityStatusByCategorys?.map(
            (data) => data?.activityType
          )
        );
      } else {
      ...
      let stackedTemp = [];
      let keysForChart = [];
      addOnServicesValues?.approvedAddOnActivitiesByCategoryAndMonth?.map(
        (stack, index) => {
          let values = stack.types;
  ...
  const getAcvityAndServicesWithParameter = async () => {
    if (!isMyReport) {
      const { data: chartDataValues } = await GET(
        `timesheet-management-service/report/activityServiceReport?startDate=${
          dataToUseInReport?.from
        }&endDate=${dataToUseInReport?.to}&contracts=${
          dataToUseInReport?.selectedContracts || []
        }&users=${
          dataToUseInReport?.selectedContractedServiceProvider || []
        }&sites=${dataToUseInReport?.selectedSites || []}&departments=${
          dataToUseInReport?.selectedDepartments || []
        }`
      );
      setActivitiesOrServicesValues(chartDataValues);
    } else {
  ...
  const getCompensationCostAnalysis = async () => {
    const { data: chartData } = await GET(
      `timesheet-management-service/timesheet/compensationCostAnalysis?contractId=${dataToUseInReport?.selectedContracts}`
    );
  ...
  const getPayments = async () => {
    ...
    if (!isMyReport) {
      const { data: chartDataValues } = await GET(
        `timesheet-management-service/report/paymentProcessingSummary?startDate=${dataToUseInReport?.from}&endDate=${dataToUseInReport?.to}`
      );
      chartData = chartDataValues;
    } else {
      const { data: chartDataValues } = await GET(
        `timesheet-management-service/report/myReport/paymentProcessingSummary?id=${myReportId}`
      );
      chartData = chartDataValues;
    }
    setPaymentsReportLog(chartData);
    if (
      (chartData?.paymentStats?.avgPayment !== 0 ||
        chartData?.paymentStats?.count !== 0 ||
        chartData?.paymentStats?.maxPayment !== 0 ||
        chartData?.paymentStats?.medianPayment !== 0 ||
        chartData?.paymentStats?.minPayment !== 0) &&
      chartData !== undefined
    ) {
      let temp = [];
      ...
      Object?.keys(chartData?.timesheetProcessingStatus)?.map((data, index) => {
        temp[index] = {
          key: data,
          value: Object?.values(chartData?.timesheetProcessingStatus)?.[index],
        };
      });
      setPieData(temp);
    } else {
    ...
    if (chartData?.paymentContracts?.length !== 0 && chartData !== undefined) {
      ...
      setBarChartCategories(
        chartData?.paymentContracts?.map((data) => data?.contractId)
      );
    } else {
      setBarChartSeries([]);
      setBarChartCategories([]);
  ...
  const getTimesheetProcessingSummary = async (filter) => {
    if (filter === "withoutParameter") {
      ...
      if (!isMyReport) {
        const { data: chartDataValues } = await GET(
          `timesheet-management-service/report/timesheetProcessingSummary?startDate=${dataToUseInReport?.from}&endDate=${dataToUseInReport?.to}&users=${dataToUseInReport?.selectedContractedServiceProvider}`
        );
        chartData = chartDataValues;
      } else {
        const { data: chartDataValues } = await GET(
          `timesheet-management-service/report/myReport/timesheetProcessingSummary?id=${myReportId}`
        );
        chartData = chartDataValues;
      }
      ...
      if (!isMyReport) {
        const { data: chartDataValues } = await GET(
          `timesheet-management-service/report/timesheetProcessingSummary?startDate=${dataToUseInReport?.from}&endDate=${dataToUseInReport?.to}&contracts=${dataToUseInReport?.selectedContracts}&users=${dataToUseInReport?.selectedContractedServiceProvider}&sites=${dataToUseInReport?.selectedSites}&departments=${dataToUseInReport?.selectedDepartments}`
        );
        chartData = chartDataValues;
      } else {
        const { data: chartDataValues } = await GET(
  ...
  const setTimesheetProcessingSummary = () => {
    ...
    if (timesheetProcessingSummaryData) {
      Object?.keys(timesheetProcessingSummaryData?.paymentStatus)?.map(
        (data, index) => {
          temp[index] = {
            key: data,
            value: Object?.values(
              timesheetProcessingSummaryData?.paymentStatus
            )?.[index],
          };
        }
      );
      setPieData(temp);
      let tempStackedSeries = [];
      Object.keys(timesheetProcessingSummaryData?.statusSummary?.payment)?.map(
        (data, index) => {
          tempStackedSeries.push({
            data: Object.keys(
              timesheetProcessingSummaryData?.statusSummary
            )?.map(
              (stackedData) =>
                Object?.values(
                  timesheetProcessingSummaryData?.statusSummary[stackedData]
                )?.[index]
            ),
            name: data,
          });
      ...
      setStackedCategories(
        Object.keys(timesheetProcessingSummaryData?.statusSummary)
      );
      setTotalTimesheetRejectedCount(
        timesheetProcessingSummaryData?.rejectedTimesheetCount
      );
      setTotalSubmittedTimesheets(
        timesheetProcessingSummaryData?.totalSubmittedTimesheets
      );
      setRejectedTimesheetCountBreakUp(
        timesheetProcessingSummaryData?.rejectedTimesheetCountBreakUp
      );
    }
    setIsLoading(false);
  };
  ...
  const getListingOfTimesheetNotPaid = async (filter) => {
    if (filter === "withoutParameter") {
      ...
      if (!isMyReport) {
        const { data: chartDataValues } = await GET(
          `timesheet-management-service/report/notPaidTimesheets?startDate=${dataToUseInReport?.from}&endDate=${dataToUseInReport?.to}&users=${dataToUseInReport?.selectedContractedServiceProvider}`
        );
        chartData = chartDataValues;
      } else {
        const { data: chartDataValues } = await GET(
          `timesheet-management-service/report/myReport/notPaidTimesheets?id=${myReportId}`
        );
        chartData = chartDataValues;
      }
      if (chartData) {
      ...
      if (!isMyReport) {
        const { data: chartDataValues } = await GET(
          `timesheet-management-service/report/notPaidTimesheets?startDate=${dataToUseInReport?.from}&endDate=${dataToUseInReport?.to}&contracts=${dataToUseInReport?.selectedContracts}&users=${dataToUseInReport?.selectedContractedServiceProvider}&sites=${dataToUseInReport?.selectedSites}&departments=${dataToUseInReport?.selectedDepartments}`
        );
        chartData = chartDataValues;
      } else {
        const { data: chartDataValues } = await GET(
  ...
  const getSubmittedTimesheetsPaymentStatus = async (filter) => {
    if (filter === "withoutParameter") {
      ...
      if (!isMyReport) {
        const { data: chartDataValues } = await GET(
          `timesheet-management-service/report/submittedTimesheetsPaymentStatus?startDate=${dataToUseInReport?.from}&endDate=${dataToUseInReport?.to}&users=${dataToUseInReport?.selectedContractedServiceProvider}`
        );
        chartData = chartDataValues;
      } else {
        const { data: chartDataValues } = await GET(
          `timesheet-management-service/report/myReport/submittedTimesheetsPaymentStatus?id=${myReportId}`
        );
        chartData = chartDataValues;
      }
      if (chartData) {
  ...
  const getContractRenewalReport = async () => {
    const { data: contractRenewalReport } = await GET(
      "contract-managment-service/reports/contractRenewalReport"
    );
  ...
  const getContractRenewalReportWithParameters = async () => {
    const { data: contractRenewalReport } = await GET(
      `contract-managment-service/reports/contractRenewalReport?sites=${dataToUseInReport?.selectedSites}&departments=${dataToUseInReport?.selectedDepartments}&renewalDays=${dataToUseInReport?.renewalreportingTimePeriod}&contractPolicyType=${dataToUseInReport?.contractContinuationPolicy}`
    );
    if (contractRenewalReport) {
      setContractRenewalReport(contractRenewalReport);
    }
  ...
  const getOneTimeContract = async () => {
    const { data: oneTimeContract } = await GET(
      `contract-managment-service/reports/oneTimeContractReport`
    );
    if (oneTimeContract) {
  ...
  const getOneTimeContractWithParameters = async () => {
    const { data: oneTimeContract } = await GET(
      `contract-managment-service/reports/oneTimeContractReport?renewalDays=${dataToUseInReport?.renewalreportingTimePeriod}`
    );
    if (oneTimeContract) {
      setOneTimeContract(oneTimeContract);
    }
  ...
  const getNonCompliantContractReportTile = async () => {
    const { data: nonCompliantContract } = await GET(
      `contract-managment-service/reports/documentProofReport?contractNames=${dataToUseInReport?.selectedContracts}&contractStatus=${dataToUseInReport?.contractStatus}&sites=${dataToUseInReport?.selectedSites}&departments=${dataToUseInReport?.selectedDepartments}`
    );
  ...
  const getNonCompliantContractReport = async () => {
    const { data: nonCompliantContract } = await GET(
      `contract-managment-service/reports/nonCompliantContractReport?podType=${encodeURIComponent(
        selectedPodTypeFromTile
      )}&contractStatus=${dataToUseInReport?.contractStatus}&contractNames=${
        dataToUseInReport?.selectedContracts
      }&sites=${dataToUseInReport?.selectedSites}&departments=${
        dataToUseInReport?.selectedDepartments
      }`
  ...
  const getActivitiesServicesValues = (value) => {
    ...
    reasonNotDone = [];
    reportLog
      ?.filter((data) => data?.activityStatus === value)
      ?.map((data) => {
        activityPerformed.push(data?.activityPerformed?.activity);
        startDateTime.push(
          `${format(
            new Date(data?.activityTimeFrame?.stateDate),
            "MM-dd-yyyy"
          )}, ${data?.activityTimeFrame?.startTime}`
        );
        endDateTime.push(
          `${format(
            new Date(data?.activityTimeFrame?.endDate),
            "MM-dd-yyyy"
          )}, ${data?.activityTimeFrame?.endTme}`
        );
        user
    ...
    return value === "DONE"
      ? [activityPerformed, startDateTime, endDateTime, contractProvider, ""]
      : value === "TODO"
      ? [activityPerformed, startDateTime, contractProvider, ""]
      : [activityPerformed, startDateTime, contractProvider, "", reasonNotDone];
  };
  ...
  const getAddOnActivitiesServicesValues = (value) => {
    ...
    if (value === "Rejected") {
      addOnRejectedReportLog !== [] &&
        addOnRejectedReportLog?.map((data) => {
          addonActivityServices.push(data?.activity?.activity?.activity);
          requestDateTime.push(
            `${formatInTimeZone(
              new Date(data?.activity?.createdDate),
              "America/New_York",
              "MM-dd-yyyy, HH:mm"
            )}`
          );
          rejectedDateTime.push(
            `${
              data?.logs?.filter(
                (filterData) => filterData?.workFlowAction === "REJECTED"
              ) !== []
                ? formatInTimeZone(
                    new Date(
                      data?.logs?.filter(
                        (filterData) =>
                          filterData?.workFlowAction === "REJECTED"
                      )?.[0]?.createdDate
                    ),
                    "America/New_York",
                    "MM-dd-yyyy, HH:mm"
          ...
          requestReviewer.push(
            data?.logs?.filter(
              (filterData) => filterData?.workFlowAction === "REJECTED"
            ) !== []
              ? data?.logs?.filter(
                  (filterData) => filterData?.workFlowAction === "REJECTED"
                )?.[0]?.workFlowUser?.name?.name
              : "-"
          );
      ...
      addOnAcceptedReportLog !== [] &&
        addOnAcceptedReportLog?.map((data) => {
          ...
          rejectedDateTime.push(
            `${
              data?.logs?.filter(
                (filterData) => filterData?.workFlowAction === "APPROVED"
              )?.length !== 0
                ? formatInTimeZone(
                    new Date(
                      data?.logs?.filter(
                        (filterData) =>
                          filterData?.workFlowAction === "APPROVED"
                      )?.[0]?.createdDate
                    ),
                    "America/New_York",
                    "MM-dd-yyyy, HH:mm"
  ...
  const getTimesheetProcessingSummaryValues = (value) => {
    ...
    if (value === "Not Paid") {
      timesheetProcessingSummaryData?.notPaidTimesheets?.map((data) => {
        ...
        period.push(
          `${format(
            new Date(data?.timesheet?.timesheetPeriod?.startDate) || new Date(),
            "MMM dd"
          )} - ${format(
            new Date(data?.timesheet?.timesheetPeriod?.endDate) || new Date(),
            "MMM dd yyyy"
          )}`
        );
        approvalDate.push(
          `${format(
            new Date(
              data?.activityLoggerList?.filter(
                (filterData) => filterData?.workFlowAction === "APPROVED"
              )?.[0]?.createdDate
            ) || new Date(),
            "MM-dd-yyyy, HH:mm"
    ...
    return [timesheet, period, approvalDate, actionBy, serviceProvider];
  };
  ...
  const getNotPaidTimesheetsValues = () => {
    ...
    notPaidTimesheetsData?.unPaidTimesheetsByContract?.map((data) => {
      data?.timesheets?.map((timesheetData) => {
        ...
        period.push(
          `${format(
            new Date(timesheetData?.timesheetPeriod?.startDate) || new Date(),
            "MMM dd"
          )} - ${format(
            new Date(timesheetData?.timesheetPeriod?.endDate) || new Date(),
            "MMM dd yyyy"
          )}`
        );
        departmentAndSite.push(
          `${
            Object.values(
              Object.values(
                timesheetData?.siteDepartmentDetails?.siteDepartmentDetailMap
              )?.[0]?.departmentMap
            )?.[0]?.name
          }, ${
            Object.values(
        ...
        currentStatus.push(
          availableTimesheetStatus[timesheetData?.timesheetStatus?.status]
        );
    ...
    submittedTimesheetsPaymentStatusData?.timesheetPayment?.map((data) => {
      ...
      departmentAndSite.push(
        `${
          Object.values(
            Object.values(
              data?.timesheet?.siteDepartmentDetails?.siteDepartmentDetailMap
            )?.[0]?.departmentMap
          )?.[0]?.name
        }, ${
          Object.values(
      ...
      submissionDate.push(
        format(
          new Date(
            data?.timesheet?.timesheetPeriod?.submissionDate || new Date()
          ),
          "d MMM yyyy"
        )
      );
      currentStatus.push(
        availableTimesheetStatus[data?.timesheet?.timesheetStatus?.status]
      );
      statusDate.push(
        format(
          new Date(data?.timesheet?.lastModifiedDate || new Date()),
          "d MMM yyyy"
        )
      );
      paymentStatus.push(data?.payment !== null ? "Paid" : "-");
      paymentAmount.push(
        data?.payment !== null ? data?.payment?.actualPayment?.payment : "-"
      );
      paymentDate.push(
        data?.payment !== null
          ? format(
              new Date(data?.payment?.paymentDate?.date || new Date()),
              "d MMM yyyy"
            )
          : "-"
      );
    ...
    if (value === "paidOnTime") {
      paymentsReportLog?.paidOnTime?.map((data) => {
        ...
        period.push(
          data?.payment
            ? `${format(
                new Date(data?.payment?.paymentPeriod?.startDate),
                "MMM d"
              )} - ${format(
                new Date(data?.payment?.paymentPeriod?.endDate),
                "MMM d yyyy"
              )} `
            : "-"
        );
        ...
        invoiceAmount.push(
          data?.payment !== null
            ? `$ ${data?.payment?.expectedPayment?.payment} `
            : "-"
        );
        paidAmount.push(
          data?.payment !== null
            ? `$ ${data?.payment?.actualPayment?.payment} `
            : "-"
        );
      });
    }
  ...
  const getCompensationCostAnalysisValues = () => {
    let leftHeadings = [
      ...
      "Add-ON",
      "Obligated Variance",
      "Contract Year Balance",
      "Contract Year Projected Balance",
      "Contract Period Balance",
      "Contract Period Projected Balance",
      "Additional Services",
      "Reduced Services",
      "Actual",
    ...
    compensationCostAnalysis?.map((data) => {
      oneColValue = [
        data?.obligatedExpected.toFixed(2),
        data?.obligatedActivitiesCosts.toFixed(2),
        data?.addOnActivitiesCost.toFixed(2),
        data?.obligatedVariance.toFixed(2),
        data?.contractYearBalance.toFixed(2),
        data?.contractYearProjectedBalance.toFixed(2),
        data?.contractPeriodBalance.toFixed(2),
        data?.contractPeriodProjectedBalance.toFixed(2),
        data?.additionalServicesCost.toFixed(2),
        data?.reducedServicesCost.toFixed(2),
        data?.totalActivitiesCost.toFixed(2),
        data?.policyBasedPayment.toFixed(2),
        data?.maxPaymentPerTimesheetSubmission.toFixed(2),
      ];
  ...
  const getContractManagementUpcomingValues = (value) => {
    ...
    let mapValue =
      value === "INDIVIDUAL" ? individualContract : multipleContract;
    ...
    mapValue?.map((data) => {
      ...
      contractManagementExpirationDate.push(
        format(
          new Date(data?.contractDetail?.contractTerm?.endDate),
          "MM-dd-yyyy"
        )
      );
      contractManagementContractingEntity.push(
        data?.contractorBusinessEntity !== null
          ? data?.contractorBusinessEntity?.businessEntity?.name
          : "-"
      );
      contractManagementPointOfContact.push(
        `${user
          ?.filter(
            (userData) =>
              userData?.id === data?.contractDetail?.contractManager?.userID
          )
          ?.map((data) => data?.name?.firstName)} ${user
          ?.filter(
            (userData) =>
              userData?.id === data?.contractDetail?.contractManager?.userID
          )
          ?.map((data) => data?.name?.lastName)} `
      );
      contractManagementPointOfContactNumber.push(
        user
          ?.filter(
            (userData) =>
              userData?.id === data?.contractDetail?.contractManager?.userID
          )
          ?.map((data) => data?.communication?.mobileNumber)
      );
      contractManagementEmail.push(
        user
          ?.filter(
            (userData) =>
              userData?.id === data?.contractDetail?.contractManager?.userID
          )
          ?.map((data) => data?.email?.officialEmail)
      );
    });
  ...
  const getContractComplianceValues = (value) => {
    ...
    let mapValue =
      value === "documentNotUploadedContracts"
        ? nonCompliantContract?.documentNotUploadedContracts
        : value === "expiredContracts"
        ? nonCompliantContract?.expiredContracts
        : value === "renewalContracts"
        ? nonCompliantContract?.renewalContracts
    ...
    mapValue?.map((data) => {
      ...
      contractCompliaceContractManager.push(
        `${user
          ?.filter(
            (userData) =>
              userData?.id === data?.contractDetail?.contractManager?.userID
      ...
      contractCompliaceExpirationDate.push(
        format(
          new Date(data?.contractDetail?.contractTerm?.effectiveDate),
          "MM-dd-yyyy"
        )
      );
      contractCompliaceContractingEntity.push(
        data?.contractorBusinessEntity !== null
          ? data?.contractorBusinessEntity?.businessEntity?.name
          : "-"
      );
      contractCompliacePointOfContact.push(
        `${user
          ?.filter(
            (userData) =>
              userData?.id === data?.contractDetail?.contractManager?.userID
      ...
      contractCompliacePointOfContactNumber.push(
        user
          ?.filter(
            (userData) =>
              userData?.id === data?.contractDetail?.contractManager?.userID
  ...
  const getHeaderValues = () => {
    ...
    headerValues.push("");
    compensationCostAnalysis?.map((data) =>
      headerValues.push(`${months[data?.month]}, ${data?.year}`)
    );
    return headerValues;
  };
    ...
    return (
      <>
        <div className={style.grid2}>
          <div>
            <div
              className={`${style.entityNameBolderStyle} ${style.textAlignLeft} ${style.marginTop20} `}
            >
              Activity / Services Status
            </div>
          ...
          <div>
            <div
              className={`${style.entityNameBolderStyle} ${style.textAlignLeft} ${style.marginTop20} `}
            >
              By Category Of Service Performed
            </div>
        ...
        <div
          className={`${style.headerBorderStyle} ${style.marginTop40} `}
        ></div>
        <div className={style.marginTop40}>
          <div
            className={`${style.entityNameBolderStyle} ${style.textAlignLeft} ${style.marginTop20} ${style.marginBottom20} `}
          >
            Trend For Activities / Services Completed
          </div>
        ...
        <div className={style.marginTop40}>
          <div
            className={`${style.entityNameBolderStyle} ${style.textAlignLeft} ${style.marginTop20} ${style.marginBottom20} `}
          >
            Percentage Of Activities / Services Completed By Category Type
          </div>
          <div className={style.reportWidthToFitFullScreen}>
        ...
        {reportLog?.filter((data) => data?.activityStatus === "DONE")
          ?.length !== 0 && (
          <>
            <ReportsTable
              ...
              tableValue={reportLog?.filter(
                (data) => data?.activityStatus === "DONE"
              )}
              activitiesServicesValues={getActivitiesServicesValues("DONE")}
              styleName={style.grid5}
            />
            <div
              className={`${style.mildBorderStyle} ${style.marginTop20} `}
            ></div>
          </>
        )}
        {reportLog?.filter((data) => data?.activityStatus === "TODO")
          ?.length !== 0 && (
          <>
            <ReportsTable
        ...
        {reportLog?.filter((data) => data?.activityStatus === "NOTDONE")
          ?.length !== 0 && (
          <>
            <ReportsTable
              ...
              tableValue={reportLog?.filter(
                (data) => data?.activityStatus === "NOTDONE"
              )}
      ...
      >
        ...
        <div>
          <ReportPerformanceAndOptions
            handle={handle}
            getIsRefresh={getIsRefresh}
            handlePrint={handlePrint}
            isUpdated={isLoading}
            dataToUseInReport={dataToUseInReport}
            refToUse={PDFRef}
            getIsDownloadClicked={getIsDownloadClicked}
            isNoData={isNoData}
          />
          <FullScreen handle={handle}>
            <div className={`Report`} ref={PDFRef}>
              <div
                className={`${style.reportBackgroundCard} ${style.marginTop20} `}
                ref={componentRef}
              >
                <table style={{ width: "100%" }}>
                  ...
                  <tbody>
                    <div className={style.justifyCenter}>
                      <div className={style.marginTop20}>
                        {reportType === "paymentsProcessingSummary" && (
                          <div
                            className={`${style.entityNameBolderStyle} ${style.textAlignCenter} ${style.marginTop5} `}
                          >
                            {dataToUseInReport?.selectedContractsToSend
                              ?.map((data) => data?.contractName?.contractName)
                              .join(", ")}
                          </div>
                        )}
                        <div
                        ...
                        {dataToUseInReport?.reportingTimePeriod !== "" && (
                          <div
                            className={`${style.reportRunByTextStyle} ${style.textAlignCenter} ${style.marginTop5} `}
                          >
                            Reporting Period used for this report :{" "}
                            {dataToUseInReport?.reportingTimePeriod} (
                            {dataToUseInReport?.fromToDisplay} to{" "}
                            {dataToUseInReport?.toToDisplay}){" "}
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    <div className={style.marginTop20}>
                      <div
                        className={`${style.entityNameBolderStyle} ${style.textAlignLeft} ${style.marginTop5} `}
                      >
                        Reporting Parameters Applied
                      </div>
                      ...
                      reportType === "oneTimeContract" ? (
                        <div className={`${style.grid2} ${style.marginTop20} `}>
                          <div>
                            <div
                              className={`${style.reportRunByTextStyle} ${style.marginTop5} `}
                            >
                              Renewal Time Frame{" "}
                            </div>
                            <div
                              className={`${style.reportTypeValueTextStyle} ${style.textAlignLeft} ${style.marginTop5} `}
                            >{`Renewal within Next ${dataToUseInReport?.renewalreportingTimePeriod} days`}</div>
                          </div>
                          <div>
                            <div
                              className={`${style.reportRunByTextStyle} ${style.marginTop5} `}
                            ...
                            <div
                              className={`${style.reportTypeValueTextStyle} ${style.textAlignLeft} ${style.marginTop5} `}
                            >
                              {dataToUseInReport?.selectedSitesToSend
                                ?.map((data) => data?.siteName?.siteName)
                                .join(", ") || "All Sites"}
                            </div>
                          </div>
                          <div>
                            ...
                            <div
                              className={`${style.reportTypeValueTextStyle} ${style.textAlignLeft} ${style.marginTop5} `}
                            >
                              {dataToUseInReport?.selectedDepartmentsToSend
                                ?.map((data) => data?.departmentName?.name)
                        ...
                        <div className={`${style.grid2} ${style.marginTop20} `}>
                          ...
                          <div>
                            ...
                            >
                              {dataToUseInReport?.selectedContractsToSend
                                ?.map(
                                  (data) => data?.contractName?.contractName
                                )
                                .join(", ") || "All Contracts"}
                            </div>
                          </div>
                          <div>
                            ...
                            <div
                              className={`${style.reportTypeValueTextStyle} ${style.textAlignLeft} ${style.marginTop5} `}
                            >{`${dataToUseInReport?.selectedContractedServiceProviderToSend?.name?.firstName} ${dataToUseInReport?.selectedContractedServiceProviderToSend?.name?.lastName}`}</div>
                          </div>
                        </div>
                      ) : reportType === "paymentsProcessingSummary" ? (
                        <div className={`${style.grid2} ${style.marginTop20} `}>
                        ...
                        <div className={`${style.grid2} ${style.marginTop20} `}>
                          ...
                          <div>
                            ...
                            <div
                              className={`${style.reportTypeValueTextStyle} ${style.textAlignLeft} ${style.marginTop5} `}
                            >
                              {dataToUseInReport?.contractStatus === "ACTIVE"
                                ? "Active"
                                : dataToUseInReport?.contractStatus === "DRAFT"
                                ? "Draft"
                          ...
                          <div>
                            ...
                            <div
                              className={`${style.reportTypeValueTextStyle} ${style.textAlignLeft} ${style.marginTop5} `}
                            >
                              {dataToUseInReport?.podType || "Select One"}
                            </div>
                          </div>
                        </div>
                      ) : (
                      ...
                      {reportType === "activitiesOrServices" ? (
                        <>
                          {activitiesOrServicesValues?.activities?.length !==
                            0 ||
                          activitiesOrServicesValues?.activityServiceReports
                            ?.length !== 0 ||
                          activitiesOrServicesValues?.activityStatusByCategorys
                            ?.length !== 0 ||
                          activitiesOrServicesValues?.completedActivitiesByDate
                            ?.length !== 0 ||
                          activitiesOrServicesValues
                            ?.completedActivitiesBycategoryAndMonth?.length !==
                            0 ? (
                            // <>
                            //     <div className={style.grid2}>
                            //         <div>
                            ...
                            <ReportNoDataBox
                              heading={
                                "Based on the parameters selected and applied, there were NO RECORDS found to include in the report."
                              }
                              subHeading={
                                "Try again by changing some of the parameters on the left. If there are any qualifying records, the report will get displayed."
                              }
                            />
                          )}
                        </>
                        ...
                        <>
                          {addOnServicesValues?.addOnActivityServiceReports
                            ?.length !== 0 ||
                          addOnServicesValues?.addOnActivityStatusByCategorys
                            ?.length !== 0 ||
                          addOnServicesValues?.approvedActivities?.length !==
                            0 ||
                          addOnServicesValues
                            ?.approvedAddOnActivitiesByCategoryAndMonth
                            ?.length !== 0 ||
                          addOnServicesValues?.approvedAddOnActivityByDate
                            ?.length !== 0 ||
                          addOnServicesValues?.rejectedActivities?.length !==
                            0 ? (
                            <>
                              <div className={style.grid2}>
                                <div>
                                ...
                                <div>
                                  <div
                                    className={`${style.entityNameBolderStyle} ${style.textAlignLeft} ${style.marginTop20} `}
                                  >
                                    By Add On Activity/ Service Type
                                  </div>
                                  <div className={style.marginTop20}>
                                    <ApexGroupedBarChart
                                      series={series}
                              ...
                              <div className={style.marginTop40}>
                                <div
                                  className={`${style.entityNameBolderStyle} ${style.textAlignLeft} ${style.marginTop20} ${style.marginBottom20} `}
                                >
                                  Percentage Of Add On Activities / Services
                                  Requests Approved
                                </div>
                                <div
                                  className={
                                    style.addOnReportWidthToFitFullScreen
                                  }
                                >
                                  {apexStackedBarChartDisplay}
                                </div>
                              ...
                              {addOnRejectedReportLog?.length !== 0 && (
                                <>
                                  <ReportsTable
                                    tableType={
                                      "List Of Request Rejected For Add On Activity / Services"
                                    }
                                    tableHeader={[
                                      "Add on Activity/ Services",
                                      "Request Date/ Time",
                                      "Rejected Date/ Time",
                                      "Requesting Provider",
                                      "Request Reviewer",
                                    ...
                                    activitiesServicesValues={getAddOnActivitiesServicesValues(
                                      "Rejected"
                                    )}
                              ...
                              {addOnAcceptedReportLog?.length !== 0 && (
                                <>
                                  <ReportsTable
                                    tableType={
                                      "List Of Approved Add On Activity / Services Requests"
                                    }
                                    tableHeader={[
                                      "Add on Activity/ Services",
                                      "Request Date/ Time",
                          ...
                          <>
                            <div className={style.grid2}>
                              ...
                              <div>
                                <div
                                  className={`${style.entityNameBolderStyle} ${style.textAlignLeft} ${style.marginTop20} `}
                                >
                                  Payments Made Summary Statistics
                                </div>
                                <div
                                  className={`${style.summaryGrid} ${style.marginTop40} `}
                                >
                                  <div>
                                    <div
                                      className={`${style.summaryTextStyle} ${style.textAlignLeft} ${style.marginTop20} `}
                                    >
                                      Minimum Payment
                                    </div>
                                    <div
                                    ...
                                    <div
                                      className={`${style.summaryTextStyle} ${style.textAlignLeft} ${style.marginTop20} `}
                                    >
                                      Average Payment Per Timesheet
                                    </div>
                                    <div
                                      className={`${style.summaryTextStyle} ${style.textAlignLeft} ${style.marginTop20} `}
                                    >
                                      Median Payment ( 50TH Percentile )
                                    </div>
                                  </div>
                                  <div>
                                    <div
                                      className={`${style.summaryTextStyle} ${style.marginTop20} `}
                                    >
                                      ${" "}
                                      {
                                        paymentsReportLog?.paymentStats
                                          ?.minPayment
                                      }
                                    </div>
                                    <div
                            ...
                            <div>
                              <div
                                className={`${style.entityNameBolderStyle} ${style.textAlignLeft} ${style.marginTop20} `}
                              >
                                Payment For Services Performed By Contract
                              </div>
                              <div className={style.marginTop20}>
                                <ApexBarChart
                                  series={barChartSeries}
                                  categories={barChartCategories}
                                  reportingPeriod={`${formatInTimeZone(
                                    new Date(
                                      dataToUseInReport?.from || new Date()
                                    ),
                                    "America/New_York",
                                    "MMM d"
                                  )} to ${formatInTimeZone(
                                    new Date(
                                      dataToUseInReport?.to || new Date()
                                    ),
                                    "America/New_York",
                                    "MMM d"
                                  )} `}
                                  yAxisTitle="Dollars Paid"
                                />
                              </div>
                            </div>
                            <div
                            ...
                            {/* <div className={`${ style.mildBorderStyle } ${ style.marginTop20 } `}></div> */}
                            {paymentsReportLog?.paymentPastDue?.length !==
                              0 && (
                              <>
                                <ReportsTable
                                  ...
                                  activitiesServicesValues={getPaymentsValues(
                                    "paymentPastDue"
                                  )}
                                  styleName={style.grid6}
                                />
                            ...
                            {paymentsReportLog?.paymentDelayed?.length !==
                              0 && (
                              <>
                                <ReportsTable
                            ...
                            {paymentsReportLog?.rejected?.length !== 0 && (
                              <>
                                <ReportsTable
                                  tableType={
                                    "Timesheets With Rejected Payments"
                                  }
                                  tableHeader={[
                                    "Timesheet",
                                    "Period",
                                  ...
                                  activitiesServicesValues={getPaymentsValues(
                                    "rejectedTimesheetPayments"
                                  )}
                                  styleName={style.grid6}
                                />
                              </>
                            )}
                            {paymentsReportLog?.paymentNotDone?.length !==
                              0 && (
                              <>
                                <ReportsTable
                        ...
                        <>
                          {totalSubmittedTimesheets !== 0 ? (
                            <div>
                              <div
                                className={
                                  style.timeSheetProcessingSummaryCalendarGrid
                                }
                              >
                                <div>
                                  <div
                                  ...
                                  <div
                                    className={style.calendarBoxBorderStyle}
                                  ></div>
                                  <div
                                    className={style.calendarBoxTopStyle}
                                  ></div>
                                  <div
                                    className={`${style.calendarBoxBottomStyle} ${style.alignCenter} ${style.justifyCenter} `}
                                  >
                                    <div>
                                      <div className={style.calendarDateStyle}>
                                        {totalSubmittedTimesheets}
                                      </div>
                                      <div
                                        className={style.calendarMonthYearStyle}
                                      >
                                        June 2023
                                      </div>
                                    </div>
                                ...
                                <div>
                                  <div
                                    className={`${style.entityNameBolderStyle} ${style.textAlignLeft} ${style.marginTop20} `}
                                  >
                                    Timesheet Processing Status Summary
                                  </div>
                                  <div>
                                    {/* <ApexStackedBarChart stackedSeries={stackedSeries} stackedCategories={stackedCategories} /> */}
                                    {apexStackedBarChartDisplay}
                                ...
                                <div>
                                  <div className={style.spaceBetween}>
                                    ...
                                    <div
                                      className={`${style.entityNameBolderStyle} ${style.textAlignLeft} ${style.marginTop20} `}
                                    >
                                      {totalTimesheetRejectedCount}
                                    </div>
                                  </div>
                                  <div className={style.marginTop20}>
                                    {Object?.keys(
                                      rejectedTimesheetCountBreakUp || {}
                                    )?.map((data, index) => (
                                      <div
                                      ...
                                      >
                                        <div className={style.spaceBetween}>
                                          <p
                                            className={style.statisticsProgress}
                                          >
                                            {getProgressValue[data]}
                                          </p>
                                          <p
                                            className={style.progressCountStyle}
                                          >
                                            {
                                              Object?.values(
                                                rejectedTimesheetCountBreakUp
                              ...
                              <div
                                className={`${style.headerBorderStyle} ${style.marginTop40} `}
                              ></div>
                              {timesheetProcessingSummaryData?.notPaidTimesheets
                                ?.length !== 0 && (
                                <>
                                  <ReportsTable
                                    tableType={
                                      "Timesheet Approved But Not Paid"
                                    }
                                    tableHeader={[
                                      "Timesheet",
                                      "Period",
                                    ...
                                    activitiesServicesValues={getTimesheetProcessingSummaryValues(
                                      "Not Paid"
                                    )}
                                    styleName={style.grid5}
                                  />
                              ...
                              {timesheetProcessingSummaryData
                                ...
                                <>
                                  <ReportsTable
                                    tableType={
                                      "Timesheets With Payment On Time"
                                    }
                                    tableHeader={[
                                      "Timesheet",
                                      "Period",
                                    ...
                                    tableValue={
                                      timesheetProcessingSummaryData?.rejectedTimesheets
                                    }
                                    activitiesServicesValues={getTimesheetProcessingSummaryValues(
                                      "Rejected"
                                    )}
                        ...
                        <>
                          ...
                          notPaidTimesheetsData?.unPaidTimesheetsByContract
                            ?.length !== 0 ? (
                            <div className={style.marginTop20}>
                              <div
                                className={`${style.entityNameBolderStyle} ${style.textAlignLeft} ${style.marginTop20} `}
                              ...
                              <div
                                className={`${style.grid4} ${style.marginTop20} `}
                              >
                                <div>
                                  <div
                                    className={`${style.timesheetsNotPaidCountContainer} ${style.alignCenter} ${style.justifyCenter} `}
                                  >
                                    {
                                      notPaidTimesheetsData?.unpaidTimesheetsCount
                                    }
                                  </div>
                                  <div
                                    className={`${style.timesheetsNotPaidTitleContainer} ${style.alignCenter} ${style.justifyCenter} `}
                                  >
                                    Timesheets Not Paid
                                  </div>
                                </div>
                                <div>
                                  <div
                                    className={`${style.timesheetsNotPaidCountContainer} ${style.alignCenter} ${style.justifyCenter} `}
                                  >
                                    {notPaidTimesheetsData?.usersCount}
                                  </div>
                                  <div
                                    className={`${style.timesheetsNotPaidTitleContainer} ${style.alignCenter} ${style.justifyCenter} `}
                                  >
                                ...
                                <div>
                                  <div
                                    className={`${style.timesheetsNotPaidCountContainer} ${style.alignCenter} ${style.justifyCenter} `}
                                  >
                                    {notPaidTimesheetsData?.contractCount}
                                  </div>
                                  <div
                                    className={`${style.timesheetsNotPaidTitleContainer} ${style.alignCenter} ${style.justifyCenter} `}
                                  >
                                ...
                                <div>
                                  ...
                                  >
                                    $
                                    {notPaidTimesheetsData?.unpaidAmount >= 1000
                                      ? `${(
                                          notPaidTimesheetsData?.unpaidAmount /
                                          1000
                                        ).toFixed(2)}K`
                                      : notPaidTimesheetsData?.unpaidAmount}
                                  </div>
                              ...
                              {notPaidTimesheetsData?.unPaidTimesheetsByContract
                                ?.length !== 0 && (
                                <>
                                  {notPaidTimesheetsData?.unPaidTimesheetsByContract?.map(
                                    (data) => (
                                      <ReportsTable
                          ...
                          {submittedTimesheetsPaymentStatusData
                            ...
                            <>
                              <ReportsTable
                                ...
                                tableValue={
                                  submittedTimesheetsPaymentStatusData?.timesheetPayment
                                }
                      ...
                      reportType === "upcomingContractRenewals" ||
                        ...
                        <>
                          {individualContract?.length !== 0 && (
                            <ReportsTable
                              tableType={
                                "Individual Service Provider Contract Renewal"
                              }
                              tableHeader={[
                                "Contract Name",
                                "Contract ID",
                              ...
                              activitiesServicesValues={getContractManagementUpcomingValues(
                                "INDIVIDUAL"
                              )}
                          ...
                          {multipleContract?.length !== 0 && (
                            <ReportsTable
                              tableType={
                                "Multiple Service Provider Contract Renewal"
                              }
                              tableHeader={[
                                "Contract Name",
                                "Contract ID",
                        ...
                        <>
                          ...
                          <div className={style.marginTop40}>
                            <div
                              className={`${style.entityNameBolderStyle} ${style.textAlignLeft} ${style.marginTop5} `}
                            >
                              Non Compliant Providers With Required Documents
                            </div>
                            <div
                              className={`${style.grid7} ${style.marginTop20} `}
                            >
                              <div
                                className={`${style.reportRunByTextStyle} ${style.marginTop5} `}
                              >
                            ...
                            >
                              <div
                                className={`${style.reportTypeValueBoldTextStyle} ${style.textAlignLeft} ${style.marginTop5} `}
                              >
                                John Doe
                              </div>
                              <div
                          ...
                          <div className={style.marginTop40}>
                            <div>
                              <div
                                className={`${style.entityNameBolderStyle} ${style.textAlignLeft} ${style.marginTop5} `}
                              >
                                Providers With Required Documents Needing
                                Compliance Within Next 30 Days
                              </div>
                              <div
                                className={`${style.grid7} ${style.marginTop20} `}
                              >
                          ...
                          <div className={style.marginTop40}>
                            <div>
                              <div
                                className={`${style.entityNameBolderStyle} ${style.textAlignLeft} ${style.marginTop5} `}
                              >
                                Providers In Compliance With Required Documents
                              </div>
                              <div
                                className={`${style.grid7} ${style.marginTop20} `}
                              >
                          ...
                          {isNonCompliantReportTileClicked ? (
                            <>
                              {nonCompliantContract
                                ?.documentNotUploadedContracts?.length !==
                                0 && (
                                <ReportsTable
                                  ...
                                  activitiesServicesValues={getContractComplianceValues(
                                    "documentNotUploadedContracts"
                                  )}
                                  styleName={style.individualServiceReportGrid}
                                />
                              )}
                            ...
                            <div
                              className={`${style.complianceGrid2} ${style.marginTop20} `}
                            >
                              {podTypes?.map((data, index) => (
                                <div
                                  className={`${style.complianceCardStyle} ${style.cursorPointer} `}
                                  key={index}
                                  onClick={() => {
                                    setIsNonCompliantReportTileClicked(true);
                                    setSelectedPodTypeFromTile(data);
                                  }}
                                >
                                  <div
                                    className={style.complianceLeftCardStyle}
                                  >
                                    <div
                                      className={style.complianPercentageStyle}
                                    >
                                      {`${
                                        nonCompliantContractTile
                                          ?.podTypePercentage?.[data] || 0
                                      }% `}
                                    </div>
                                  </div>
                                  <div
                                    className={style.complianceRightCardStyle}
                                  >
                                    <div className={style.fullWidth}>
                                      <div
                                        className={style.complianceHeadingStyle}
                                      >
                                        {data}
                                      </div>
                                      <div
                                        className={`${style.complianceListGrid} ${style.marginTop20} `}
                                      >
                                        <div
                                          className={style.redDotStyle}
                                        ></div>
                                        <div
                                          className={`${style.reportRunByTextStyle} `}
                                        >
                                          Expired
                                        </div>
                                        <div
                                          className={`${style.reportTypeValueBoldTextStyle} ${style.textAlignLeft} `}
                                        >
                                          {
                                            nonCompliantContractTile
                                              ?.podTypeTileCountMap?.[data]
                                              ?.expiredDocumentCount
                                          }
                                        </div>
                                      </div>
                                      <div
                                        className={`${style.complianceListGrid} ${style.marginTop10} `}
                                      >
                                        <div
                                          className={style.yellowDotStyle}
                                        ></div>
                                        <div
                                          className={`${style.reportRunByTextStyle} `}
                                        >
                                      ...
                                      >
                                        <div
                                          className={style.blueDotStyle}
                                        ></div>
                                        <div
                                          className={`${style.reportRunByTextStyle} `}
                                        >
                                          Document copy not on file
                                        </div>
                                        <div
                                          className={`${style.reportTypeValueBoldTextStyle} ${style.textAlignLeft} `}
                                        >
