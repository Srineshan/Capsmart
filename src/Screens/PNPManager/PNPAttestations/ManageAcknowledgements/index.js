...
const ManageAcknowledgement = () => {
    ...
    const getAttestationList = async (signal) => {
        ...
        if (selectedOption === 'completed') {
            url = `policy-and-procedure-management-service/policyAndProcedures/signOff?tab=level-1&role=${sessionStorage.getItem('workModeType')}&assignedUserIds=${loggedInUser}&status=${selectedOption}&noOfDays=${30}&sortBy=${sortValue}&sortByField=${sortField}&offset=${page - 1}&limit=${limit}&isPaginationRequired=${isPaginationRequired}`
        } else {
            url = `policy-and-procedure-management-service/policyAndProcedures/signOff?tab=level-1&role=${sessionStorage.getItem('workModeType')}&assignedUserIds=${loggedInUser}&status=${selectedOption}&sortBy=${sortValue}&sortByField=${sortField}&offset=${page - 1}&limit=${limit}&isPaginationRequired=${isPaginationRequired}`
        }
